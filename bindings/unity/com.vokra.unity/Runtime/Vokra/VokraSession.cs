// VokraSession.cs — high-level wrapper over a Vokra C ABI session (M2-11-T09).
//
// Promoted from examples/unity-demo/Assets/Scripts/Vokra/VokraSession.cs
...
// exposes the everyday surface and keeps every raw IntPtr and status code behind
// SafeHandle + VokraException.
//
// Thread-safety: FR-API-03 (Session is Send + Sync, atomic ref count) is v0.1
// MVP scope. Retain() clones the underlying Arc and returns an independent
// VokraSession that can be moved to another thread. Callers that stay on a
// single worker thread also satisfy the thread-local vokra_last_error contract.
...
namespace Vokra
{
    public sealed class VokraSession : IDisposable
    {
        ...
        /// session is safe to move to another thread.
        /// </summary>
        public VokraSession Retain()
        {
            VokraStatus status = NativeMethods.SessionRetain(_handle.DangerousGetHandle(), out IntPtr raw);
            VokraException.ThrowIfError(status, "session retain");
            if (raw == IntPtr.Zero)
            {
                throw new VokraException(VokraStatus.Other, "session handle was NULL on success");
            }
        ...
        public string Transcribe(float[] pcm, int sampleRate)
        {
            ...
            // call and is only Disposed later on this same worker thread.

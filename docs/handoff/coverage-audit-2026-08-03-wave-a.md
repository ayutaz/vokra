...
| 4 | dnsmos-p808-p835 | (in progress) | ⏸ prep script hanging | ONNX 4 files (`model_v8.onnx` + `sig_bak_ovr.onnx` + `sig.onnx` + `bak_ovr.onnx`) 全 fetch OK、`~/checkpoints/dns-challenge/DNSMOS/DNSMOS/`。prep script `uv run --project tools/parity python tools/parity/dnsmos_prepare_checkpoint.py --p808 ... --p835 ... --output ...` が 2 min+ hang → background task blfioc10u。**owner 直接 python + venv activate で再試行推奨** |

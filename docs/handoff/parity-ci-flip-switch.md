...
| sbv2 | `.github/workflows/parity-sbv2-real.yml` | `VOKRA_SBV2_UTMOS_ENABLE` (gates the tail-position UTMOS quality assertion — WP-24) | sidecar-hash gate on the **three** GGUF inputs consumed by today's JA/EN numerical dumper: `tests/fixtures/sbv2/sbv2-v2-multilingual-base.gguf.sha256`, `deberta-v2-large-japanese-char-wwm.gguf.sha256`, `deberta-v3-large.gguf.sha256` (main + JA BERT + EN BERT). The published `chinese-roberta-wwm-ext-large.gguf.sha256` sidecar belongs to the separate WP-19 4-file loader contract; no ZH reference dump/parity leg exists yet, so it deliberately does not open or block this JA/EN gate. Set `VOKRA_SBV2_UTMOS_GGUF` after populating `VOKRA_SBV2_UTMOS_ENABLE=1` to opt the tail UTMOS delta leg on. | sbv2 (Style-Bert-VITS2 v2 multilingual base) | SBV2 v2 plan follow-up. Cron Mon 07:15 UTC. Follows the Kokoro/Whisper **sidecar-hash gating** pattern rather than the `_ENABLE`-variable pattern: dispatch or cron always runs the `setup` job, which opens the current JA/EN parity leg only when all three required fixture sha256 sidecars are populated. The job uses `tools/parity/sbv2/uv.lock`, downloads all three pinned revisions, verifies each regenerated GGUF against its sidecar, feeds the pinned local BERT snapshots to the upstream dumper, and runs the named non-ignored full-manifest consumer. VAST proved that exact numerical path on 2026-08-18; the first corrected Actions verdict is still pending. Absent sidecars → visible `parity-sbv2-real fixture gate: CLOSED (clean skip)` summary, never a green pass. WP-24 UTMOS remains an additional opt-in. The 4-file loader still needs a ZH reference dumper and numerical leg before its fourth input can be counted as real-weight parity. See `docs/handoff/parity-sbv2-real-vast-2026-08-18.md`. |

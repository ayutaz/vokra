...
| sbv2 | `.github/workflows/parity-sbv2-real.yml` | `VOKRA_SBV2_UTMOS_ENABLE` gates the tail UTMOS quality assertion. The default JA leg requires the three main + JA/EN BERT sidecars. A manual `include_zh=true` dispatch, or an explicitly enabled future `VOKRA_SBV2_ZH_ENABLE=1` schedule, additionally requires `chinese-roberta-wwm-ext-large.gguf.sha256` and selects the four-file ZH leg. | sbv2 (Style-Bert-VITS2 v2 multilingual base) | Cron Mon 07:15 UTC remains the established three-file JA leg unless the ZH schedule variable is explicitly enabled. Both numerical paths are now real-weight proven: PR #33 run `32069035448` supplied the corrected Actions JA verdict; VAST commit `e564186` supplied the four-file ZH verdict (`1/1`, no bound changes, ZH BERT max |Δ| `1.907349e-5`). Matching PRs keep the default workload unchanged; an owner can exercise ZH with workflow dispatch `include_zh=true` after reviewing runner cost. UTMOS remains a separate opt-in, and fixture-only ZH G2P replay is not production Mandarin G2P validation. See `docs/handoff/parity-sbv2-real-vast-2026-08-18.md`. |

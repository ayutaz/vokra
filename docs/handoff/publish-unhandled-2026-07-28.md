...
| **RMVPE (F0)** | `yxlllc/RMVPE` fork of `Dream-High/RMVPE` | MIT | ☑ Commercial (2026-07-30 yousan、依頼者許可 = CC 判断) | **CC-side partial**: 内部 U-Net + GRU forward は owner real-checkpoint parity wave に defer (loud-partial = `RMVPE::extract_real` returns `VokraError::UnsupportedOp`)。converter + config + real mel front-end + 360-class → Hz decoder + parity harness scaffold は landed | (a) owner が real checkpoint 用意 → parity dumper 作成 + `parity_rmvpe.rs` env `VOKRA_RMVPE_REAL_GGUF` 発火 or (b) CC 別 wave で internal forward 実装 (owner GO 待ち) |

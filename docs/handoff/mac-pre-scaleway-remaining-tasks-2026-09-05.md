...
The live, read-only Hugging Face audit was refreshed on 2026-09-09 without
...
|---|---:|---:|
| Mac CPU | 131 | 45 partial + 17 no-runtime-binder + 1 non-artifact |
...
|---|---|
| `vokra/audiogen-medium` | The T5 candidate repository/revision and Apache-2.0 metadata are fixed, but the upstream AudioGen release records only mutable `t5-large`, so the historical linkage to that revision is unverified. The 16-kHz EnCodec release-file identity and scalar config are fixed, but its internal checkpoint provenance and tensor manifest remain unauthenticated. Obtain those immutable upstream facts (and the dependency/license decision), then replace the intentionally non-executable LM-only artifact with a complete composite and run VAST CPU parity. Scaleway cannot supply the missing provenance. |
...
| `vokra/moss-audio-8b-instruct` | Distinct 8B topology and strict 901-tensor legacy token-level binder are implemented. The public artifact still has the historical `moss_tts`/placeholder stamp and lacks authenticated tokenizer/chat/processor sidecars, so CLI string generation remains fail-closed. Run owner-authorized corrected conversion, independent real-weight VAST CPU/reference parity and final Apple CPU/Metal/no-fallback verification. |
...
|---|---|
| `vokra/audioldm2` | The Base model, Diffusers source revision, projection/VAE/UNet/vocoder/language-model/two-text-encoder/scheduler/tokenizer/feature-extractor roles and fail-closed bundle requirement are pinned. A dedicated Python-3.12 reference lock, complete authenticated component-tree/tensor manifest, owner license decision and official real-weight reference are still absent; only then can the projection/scheduler/full diffusion composite be implemented and measured on VAST. |
| `vokra/audioldm2-large` | The Large release is separately pinned and rejected unless its own complete authenticated bundle is supplied. It shares the missing dedicated reference lock, component-tree/tensor evidence, owner decision and official VAST reference boundary with Base; Base facts must not be reused as Large evidence. |
| `vokra/canary-qwen-2.5b` | The current strict inspector/approval/runner/binder stays inspection-only before model acquisition. Native SALM still lacks authenticated FastConformer-to-adapter-to-Qwen tensor shapes and connection semantics; tokenizer behavior/token semantics, dependency and dataset provenance are also unapproved. Obtain those primary facts and real-weight VAST reference evidence before implementing the native forward. |
| `vokra/chatterbox-multilingual-v3` | Its fixed model/source identities, component tree and T3 manifest are variant-specific, but voice encoder, S3 tokenizer/S3Gen, HiFT, watermark/conditioning and PCM composition are not authenticated as a complete manifest. Close dependency/license and watermarked voice-cloning policy, then build and independently measure the full VAST reference path. |

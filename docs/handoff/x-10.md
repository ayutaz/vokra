...
**2026-08-20 closure update:** the planned 4-week advisory soak completed
without observed false positives. `pins-sync-check.yml` now runs on every PR,
hard-fails on drift, and its stable `pins.yaml ↔ workflow sync` context is in
`main` branch protection. The paragraph below remains the restore path; it is
not an instruction to return the check to advisory mode.

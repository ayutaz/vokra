# SBV2 JA/EN full-manifest real parity on VAST (2026-08-18)
...
This closes the old implementation/runtime uncertainty for the current JA/EN
path. It does **not** constitute a GitHub Actions verdict, a ZH numerical
parity result, or an enabled UTMOS quality result.
...
`VOKRA_SBV2_UTMOS_ENABLE` was not set. The current reference CLI supports JA
and EN; it does not provide the separate ZH dump/harness needed to claim
four-file numerical parity.
...
The workflow now uses the dedicated uv lock, pins all three revisions, passes
the pinned local BERT snapshot paths into the dumper, verifies all three
regenerated hashes, and explicitly runs the named non-ignored test.

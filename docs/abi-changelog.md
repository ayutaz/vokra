...
### 2026-08-27 — 1.0.0-rc.1-dev (Qwen3-ASR strict conversion and checkpoint binding)
...
| backfill | `vokra.qwen3_asr.*` | **28** keys — 26 topology keys (`audio.conv_chunksize`, `audio.d_model`, `audio.downsample_hidden_size`, `audio.ffn_dim`, …) plus `source_revision` and `tensor_manifest_sha256` | `u32` + `f32` + `bool` + `string` | persisted | `qwen3_asr` — written by `crates/vokra-convert/src/models/qwen3_asr.rs`. The 26-key topology group was introduced by `02664f6`; the two immutable source-identity keys are additive in the 2026-08-27 strict streaming-converter wave. No pre-existing `vokra.*` key was renamed or changed meaning. | `02664f6` (topology, 2026-08-06) + (TBD) (identity) |

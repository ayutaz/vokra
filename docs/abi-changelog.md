...
| GGUF component provenance | `vokra.cosyvoice2_flow.{component,upstream_revision,checkpoint_file,checkpoint_bytes,checkpoint_sha256,config_file,config_bytes,config_sha256,config_git_blob_sha1,source_repo,source_revision,source_license_file,source_license_bytes,source_license_sha256,source_license_git_blob_sha1,source_license_declared,source.{path}.sha256,source.{path}.git_blob_sha1,tensor_manifest_sha256,data_pickle_sha256,storage_manifest_sha256,source_closure_manifest_sha256,prepared_input.bytes,prepared_input.sha256,prepared_input.authentication_status}` | Added the inspection-only CosyVoice2 flow-component provenance, per-`SOURCE_ROLES` source-closure hashes, and prepared-input authentication keys emitted by the converter | Additive; the private flow converter stamps fixed source/checkpoint/config and per-role closure identities, while composite/runtime and publication approval remain pending |
...
| backfill | `vokra.sbv2.*` | **32** keys — `converter_zero_defaults`, `d_bert`, `d_ff`, `d_model`, …  | `u32` + `f32` + `bool` + `string` | persisted | `sbv2-v2-jp-extra-base`, `litagin/Style-Bert-VITS2-2.0-base-JP-Extra` — written by `crates/vokra-convert/src/models/sbv2.rs`. **Chronology**: the `vokra.sbv2.*` group was introduced/backfilled at `f7af1ba`; the strict model/source identity was corrected at `0ee8359c`. That identity correction did not rename or change the meaning of any `vokra.sbv2.*` key. | `f7af1ba` (2026-07-28); `0ee8359c` (identity correction) |

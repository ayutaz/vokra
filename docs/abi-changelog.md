...
| `vokra-models::nsnet2` / `gguf:vokra.nsnet2.*` | canonical Microsoft 20 ms topology | Changed / Breaking artifact schema | `n_bins=161`, `n_fft=320`, `win_length=320`; exact 14 F32 tensors renamed from the pinned ONNX manifest; separate ONNX GRU input/recurrent biases | Replaces the incorrect 257-bin/512-FFT pass-through artifact contract with the official graph and `linear_before_reset=1` arithmetic. Historical public GGUFs fail closed and require reconversion. | yes (artifact) | (TBD) |
...
| `vokra-models::pyannote` | `PyanNet::{with_backend,backend}`, `PYANNOTE_HOT_OPS` | Added | explicit backend selector for SincNet/BiLSTM/linear/classifier operations | Adds a complete learned-primitive GPU route but does not remove the existing real-forward parity opt-in; the architecture remains routed-partial by default. | no | (TBD) |

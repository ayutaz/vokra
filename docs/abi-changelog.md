...
|---|---|---|---|---|---:|---|
| `vokra-convert::models::speecht5` | `convert_speecht5_file_with_tokenizer`, `SpeechT5Report`; existing `convert_speecht5_file` | Added/tightened | `(input, output, license, spm_char_model) -> Result<SpeechT5Report, ConvertError>`; compatibility entry point now returns an explicit tokenizer-required error | Requires the fixed upstream revision, exact 393 F32 tensor names/shapes, exact 81-piece tokenizer and MIT provenance metadata. The preparation wrapper removes only five exact scalar integer BatchNorm training counters. Arbitrary/partial checkpoints and tokenizer-less conversion fail before output | yes for the formerly loose offline conversion input contract | (TBD) |
| `gguf:vokra.speecht5.*` | 40-key identity/topology/tokenizer group | Added/tightened | immutable hashes and source revision; typed topology/generation scalars; exact tokenizer strings, scores and token IDs | The original 13 topology keys keep their meanings. New artifacts carry 27 additive keys. A historical tokenizer-less public file is not promoted to executable compatibility | no for canonical new conversion; legacy output remains explicitly unsupported | (TBD) |
...
| backfill / strict conversion contract | `vokra.speecht5.*` | **40** keys — the original 13 topology keys plus fixed source revision/weight/393-tensor-manifest/tokenizer hashes, omitted-counter count, remaining topology/generation scalars and `tokenizer.{scheme,kind,pieces,scores,unk_id,bos_id,pad_id,eos_id,vocab_size}` | `string` + `u32` + `f32` + `string[]` + `f32[]` | persisted | `speecht5-tts`, `microsoft/speecht5_tts` — written by `crates/vokra-convert/src/models/speecht5.rs`. The original 13 keys retain their meanings; 27 keys are additive. New conversion requires the fixed 393-F32 inference manifest and exact 81-piece tokenizer. The historical public GGUF lacks the tokenizer/new identity group and remains explicitly non-executable pending a gated replacement. | `02664f6` (2026-08-06); working tree (2026-08-26) |

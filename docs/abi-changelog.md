...
| `vokra-models::moss_audio` | `MossAudioCheckpoint`, `MossAudio`, `MossAudioVariant`, fixed topology structs, `MossAudioEmbeddings`, `MOSS_AUDIO_ENCODER_HOT_OPS` | Added | exact 4B/8B 901-tensor mmap bind; 16 kHz PCM to primary plus three text-width DeepStack embedding matrices | historical `moss_tts` stamp is exact-manifest-only; audio tower/adapters use one explicit CPU/Metal backend; Qwen3 text decode and quantized artifacts fail loudly | no | working tree (2026-08-27) |

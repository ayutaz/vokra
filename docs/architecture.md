...
| `vokra-ops` <!-- anchor: crates/vokra-ops/src/lib.rs --> | The speech operators: STFT / iSTFT / mel filterbank / MFCC / DCT and the rest of the audio dialect (`FR-OP-*`), plus the CPU FFT lowering — a from-scratch Rust reimplementation of the pocketfft algorithm rather than a bound C library. Shared primitives live here rather than in any one model: the ViT audio encoder (2-D patch embedding + pre-norm Transformer) that every self-supervised encoder binder sits on, WPE dereverberation, YIN / PyIN, inverse text normalization, and the Snake / anti-aliased-upsample vocoder pieces. |
...
| `vokra-backend-coreml` <!-- anchor: crates/vokra-backend-coreml/src/lib.rs --> | Apple ANE whole-submodel delegate (M5-01, macOS / iOS, opt-in `coreml` feature): raw Objective-C runtime + CoreML FFI, no binding crate. A hash-bound `.mlmodelc` sidecar executes the complete Whisper encoder through `DelegateBackend`; per-op `Backend` coverage stays empty by design. The 2026-08-24 M1 bakeoff passed placement but failed parity and 2x speed, so no C selector is exported. |

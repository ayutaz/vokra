...
#### 1.4 Deployment-side disclosure requirement (automatic watermark integration が Deferred の期間、必須運用)

§1.1 の machine-readable marking policy（AudioSeal / C2PA / SynthID / SilentCipher）は §8 実装状況のとおり **自動統合が Deferred**（`crates/vokra-core/src/compliance/watermark.rs` の `WatermarkConfig::backend_status()` は `WatermarkBackendStatus::Deferred` を返し、通常の TTS/VC runtime は生成音声へ自動では watermark を埋め込まない）。AudioSeal 単体の明示 embed/detect API は CPU/Metal で利用できるが、これを呼ばない生成経路へ自動適用されるものではなく、法的十分性や各変換後の検出可能性も保証しない。この期間、**Vokra の default 経路で常に提供される cue はなく、deployer 側 UI の visible AI-generated disclosure が必須**である。standalone AudioSeal を組み込む deployer も visible disclosure を省略せず、適用範囲と検出可能性を別途検証する。本節は §1.2 の deployer 責任・§1.3 の README 記述・§11 の deployer checklist に散在する運用義務を、自動統合が復活するまでの **MUST 運用要件**として集約する（法的分析の新規追加ではなく、既存要件の consolidation）。

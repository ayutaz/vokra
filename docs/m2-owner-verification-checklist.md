...
| M2-12 | 言語バインディング（Python 初回） | ⚠️ source 実装は current ABI と完全同期済み。package root export、4 native wheel の build/repair/arch smoke も実装済み | final-head wheel 検証 + § 5（合意）+ § 6（PyPI reservation / publication）|

...
- **M3-02 Vulkan** (~28% 残、Wave 11 で ADR = ~28% 維持): ~~T14-T22 実 kernel の glslc 生成 + `kernels/precompiled/*.spv` commit~~ = **完了**（2026-07-19 時点。12 `.spv` が commit 済 + `PROVENANCE` が glslangValidator 11:16.4.0 を pin + `SHA256SUMS` + `spirv.rs` の `include_bytes!` swap、`cargo test -p vokra-backend-vulkan --lib spirv::` = 24 passed / 0 failed。「developer が `brew install glslang` してから」という前提は解消済）
  <!-- claim-evidence: crates/vokra-backend-vulkan/kernels/precompiled/PROVENANCE#glslangValidator --> / eval.rs の Copy/Add 以外 op arm の SPIR-V dispatch (Wave 9 で Copy+Add 完了、Wave 10 で CI wiring 完了、Wave 11 で ADR 正式 land) / T27-T29 graph-executor 他 op Vulkan arm / T33/T34 Whisper base parity harness (`gpu-vulkan-parity.yml` scaffold 済、初回 workflow_dispatch は owner) / T37 Android arm64-v8a real-device runner / T39-T40 Android 実機 RTF (M3-18 と併走)
...
- **M3-10 Voxtral** (~2% 残): ~~(a) real Voxtral safetensors inspection → `adapter.json` side-car 作成~~ = **完了**（`12e574e`: 762/762 tensor byte-identical 変換 + 実 GGUF ロード成功 + `adapter.json` 納品済。projector は `frame_stack = 4` を表現する `FrameStackMlp` として実装済）、~~(b) T19/T20 fixtures~~ = **完了**（`tests/parity/voxtral/` に manifest + 6 binary が実在し、`pcm_source = tests/fixtures/audio/jfk-30s.wav` / `pcm_sha256 58adb4ea…` 由来の実 `greedy_text`（JFK 実文）を保持。dumper `tools/parity/dump_voxtral_reference.py` も land 済。audio encoder は **`n_layer` pre-norm transformer stack が実装済**で conv-stem stub ではない）、**(c) 実 multilang WER 実測 (T22) = 依頼者（残）**
  <!-- claim-evidence: crates/vokra-models/src/voxtral/audio_encoder.rs#pre-norm transformer -->
  <!-- claim-evidence: tests/parity/voxtral/manifest.txt#greedy_text -->、(d) **Wave 10.1 / M4**: full device-residency の per-step 1 command buffer 化 = 実 fused Metal MSL / CUDA PTX kernel (Mistral RMSNorm + RoPE + GQA-attn + SwiGLU の融合実装) = M4 kernel fusion チケット (Wave 10 で API surface + ResidencyMode enum + tests + plumbing seam は完了)
...
| M3-16 | v0.9 ABI 変更点の changelog 記録（凍結は M4-12 へ移動 → 2026-07-14 v-label 再割当 #2 で M5-13 = v1.0 GA タグへ再移動）| ✅ **Wave 1 実装 land**（`f864ade`、`docs/abi-changelog.md` schema + `docs/abi/vokra.h.v0.9-baseline.symbols` machine-anchor + `scripts/check-abi-changelog.sh`（verify/list/update-snapshot/self-test/help modes、zero-dep = bash+awk+grep+diff）+ cbindgen banner に M3-16/M4-12 参照）| — |

...
**CC-side status**（2026-07-09 更新、branch `feat/m3-plan-and-wave1`、**Wave 1〜Wave 11 の CC 実装分完了 = 19 WP 中 16 コミット済 + partial WP を Wave 7/8/9/10/11 で追撃**）:

**ticket spec**: `docs/tickets/m3/` (gitignore) に 19 WP 全 file (M3-01〜M3-19) + README 完備、~340 tickets / 285h の内訳確定 (ultracode workflow 経由)。

**実装コミット (16 WP)** — feat/m3-plan-and-wave1 で main から **45 commits ahead**、**+53,962 lines**:
...
**Verify (2026-07-09、Wave 11 merge 後)**: cargo build clean (全 12 crate + vokra-backend-vulkan opt-in feature、default / metal / cuda / vulkan feature 全 build 成功) / cargo test 全体 **1737 passed / 0 failed / 4 ignored** (default) / **1760 passed** (all features `vulkan,metal,cuda`) + **vokra-server 164 tests** (excluded workspace、Wave 10 baseline 161 から +3) + **vokra-godot 46 tests** (excluded workspace、Wave 11 で新規、compile-time layout + trampoline smoke) (Wave 10 baseline 1734/1757 から **+3 / +3**、Wave 5 baseline 1386 から累積 **+351 / +374**) / cargo fmt --check clean / cargo clippy `-D warnings` clean / `scripts/check-zero-deps.sh` OK (root Cargo.lock は vokra-* only、NFR-DS-02 保存) / `scripts/check-abi-changelog.sh` OK (Wave 11 も Rust surface + excluded workspace + docs/adr のみ、C ABI 変更なし、vokra_stream_interrupt (M3-14) のみ)。
...
- **M3-11 Godot** (~10% 残、Wave 11 で T01-T13 完了): T14-T18 = AssetLib package layout + CI/CD + NVIDIA non-bundle scanner + Android/iOS scaffold / **T19 = 実 Godot 4.3+ editor での dispatch verify (依頼者 runtime verification、M3-18 と併走)** / T20 = release note
- **M3-15 Whisper beam** (~5% 残、Wave 11 で server 側 = 100%): Whisper `BeamSearchConfig` に `no_repeat_ngram_size` field 追加 + `beam_search` 内 filter 実装 (現在は explicit UnsupportedOp、CC 実装可能、blocking ではない、将来 unlock)
...
## Summary 進捗表（2026-07-09 更新、Wave 1〜Wave 11 land 完了時点、branch `feat/m3-plan-and-wave1`）
...
**Wave 1〜Wave 11 完了サマリ（2026-07-09）**: **19 WP 中 16 WP CC 実装コミット完了 + partial WP を Wave 6/7/8/9/10/11 で追撃**（Wave 1 = 4 WP / Wave 2 = 3 WP / Wave 3 = 2 WP / Wave 3.5 = 3 WP / Wave 4 = 1 WP / Wave 5 = 3 WP scaffold / Wave 6 = M3-12 新規完成 + M3-04 GPU 完成 + M3-02/09/10 partial 進捗 / Wave 7 = M3-02 Vulkan runtime object stack + M3-09 LLM backbone scaffold + M3-10 autoregressive greedy decode 完成 / Wave 8 = M3-10 audio adapter framework (real ASR 昇格) + M3-02 SPIR-V blob 生成方針確定 + hand-crafted smoke kernel / Wave 9 = M3-02 add_f32 + eval_op(Copy/Add) + graph-executor Vulkan arm + M3-09 LLM forward body + M3-10 beam search + M3-10 GPU decode session / Wave 10 = investigation で判明した 7 タスク bundle (correctness bug + M3-15 Voxtral beam server + M3-02 CI wiring + M3-10 full residency plumbing) / **Wave 11 = investigation #2 で判明した 3 タスク = M3-02 ADR 正式 land (Wave 10 で cherry-pick 失敗した gitignore doc を直接 write で再構成) + M3-15 Whisper beam server 対称配線 + M3-11 Godot ClassDB T05-T13 (GDExtensionClassCreationInfo3 layout pin + trampoline + panic→Error 変換)**）。branch `feat/m3-plan-and-wave1`、main から **45 commits ahead / +54.0k lines**。**Wave 6/7/8/9/10/11 いずれも ultracode parallel workflow (worktree 隔離)**: Wave 6 = `wf_25308de8-a59` (5 agents 32 分、1.36M tokens、conflict 0) / Wave 7 = `wf_a0a8749a-3e0` (3 agents 31 分、917K tokens、conflict 0) / Wave 8 = `wf_6016f4e1-172` (2 agents 31 分、610K tokens、conflict 0) / Wave 9 = `wf_05152309-fec` (4 agents 30.6 分、1.15M tokens、軽微 conflict) / Wave 10 = `wf_b7fc9cf0-306` (3 agents bundle 31.7 分、789K tokens、軽微 conflict) / **Wave 11 = `wf_a7a30893-d65` (3 agents 24.7 分、471K tokens、A1 = M3-02 ADR は workflow schema エラーで直接 write 対応、B + C は conflict 0)**。**verify 全 green**: cargo build clean（全 12 crate + vokra-backend-vulkan opt-in default OFF、default / metal / cuda / vulkan feature 全 build 成功）／cargo test 全体 = **1737 passed / 0 failed / 4 ignored** (default) / **1760 passed** (all features `vulkan,metal,cuda`) + **vokra-server 164 tests** (Wave 10 baseline 161 から +3) + **vokra-godot 46 tests** (excluded workspace、Wave 11 で新規)（Wave 10 baseline 1734/1757 から **+3 / +3**、Wave 5 baseline 1386 から累積 **+351 / +374**）／cargo fmt --check clean／cargo clippy `-D warnings` clean／`scripts/check-zero-deps.sh` OK（root Cargo.lock は `vokra-*` のみ、NFR-DS-02 保存）／`scripts/check-abi-changelog.sh` OK（Wave 11 も Rust surface + excluded workspace + docs/adr のみで C ABI 変更なし）。**残 CC WP** = **なし** (Wave 11 まで通算で全 CC-implementable 分を land)。**残依頼者専任 WP** = **M3-18** + **M3-19** + **M3-12-T14 実機 sanity run** + **M3-10 real adapter.json side-car 作成 + WER 実測** + **M3-11-T19 実 Godot editor での runtime dispatch verify**。**Partial 実装 WP の依頼者引き渡し分**（follow-up、blocking ではない）= M3-02 ~28% 残 (T14-T22 実 kernel glslc 生成 + T33/T34 lavapipe parity + T39-T40 Android 実機 owner、Wave 11 で ADR 正式 land) / M3-09 ~15% 残 (T02 real HF tensor manifest 提供後 real weights 対応 owner) / M3-10 ~2% 残 (real adapter.json + WER 実測 owner、full device-residency plumbing は Wave 10 で完了、実 fused kernel は M4 follow-up) / M3-11 ~10% 残 (T14-T18 AssetLib + CI/CD + Android/iOS scaffold + T19 real Godot editor verify owner) / M3-15 ~5% 残 (Whisper `no_repeat_ngram_size` core 側実装、CC 実装可能、blocking ではない) / M3-01 RTF gate（M2-14 defer）。

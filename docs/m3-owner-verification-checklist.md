...
**CC-side status**（2026-07-09 更新、branch `feat/m3-plan-and-wave1`、**Wave 1〜Wave 7 の CC 実装分完了 = 19 WP 中 16 コミット済 + 3 partial WP を Wave 7 で追撃**）:

**ticket spec**: `docs/tickets/m3/` (gitignore) に 19 WP 全 file (M3-01〜M3-19) + README 完備、~340 tickets / 285h の内訳確定 (ultracode workflow 経由)。

**実装コミット (16 WP)** — feat/m3-plan-and-wave1 で main から **25 commits ahead**、**+27,600 lines**:
...
**Verify (2026-07-09、Wave 7 merge 後)**: cargo build clean (全 12 crate + vokra-backend-vulkan opt-in feature、default / metal / cuda / vulkan feature 全 build 成功、Android arm64 cross-build for vulkan clean) / cargo test 全体 **1542 passed / 0 failed / 4 ignored** (Wave 6 baseline 1490 から **+52 tests**、Wave 5 baseline 1386 から累積 **+156 tests**) / cargo fmt --check clean / cargo clippy `-D warnings` clean / `scripts/check-zero-deps.sh` OK (root Cargo.lock は vokra-* only、NFR-DS-02 保存) / `scripts/check-abi-changelog.sh` OK (M3-14 の `vokra_stream_interrupt` は abi-changelog に entry 記録済、Wave 6/7 は C ABI 変更なし)。

**Partial 実装 WP の残 ticket** (各 WP の spec file を参照して follow-up、blocking ではない):
- **M3-02 Vulkan** (~45% 残、Wave 7 で ~65% → ~45%): T13-T22 SPIR-V blob 本体 (GEMM/GEMV/softmax/softmax_causal/layer_norm/gelu/conv1d/elementwise/activation/transpose/gather kernel、developer 側 glslc compile → kernels/precompiled/\*.spv + SHA-256 pin) / T23/T24 Backend::supports + Backend::eval_op arms 各 op (現在は全 op UnsupportedOp) / T26-T29 graph-executor Vulkan arm / T32-T34 Whisper base parity on lavapipe / T36-T38 CI wiring (lavapipe runner + Android arm64 cross-build job + NNAPI shim ban in deny.toml) / T39-T40 Android 実機 RTF (M3-18 と併走)
...
- **M3-10 Voxtral** (~20% 残、Wave 7 で ~40% → ~20%): (1) audio adapter (encoder→text decoder soft-prefix projection) — GGUF converter への adapter weight 書き出し + runtime forward 配線 (Wave 7 で honest limitation として明記済、次 wave 継続候補) / (2) Metal/CUDA VoxtralDecodeSession (allow_device_session=true path、GPU seam ticket) / (3) beam search (現在 greedy のみ) / (4) 実 multilang WER 実測 (T22、real HF weight + real audio dataset = 依頼者)
...
## Summary 進捗表（2026-07-09 更新、Wave 1〜Wave 7 land 完了時点、branch `feat/m3-plan-and-wave1`）
...
| M3-02 | Vulkan バックエンド | ✅ **Wave 5 + Wave 6 + Wave 7 追撃**（`d11fac2` + `3549e34` + `2ff5561`、`vokra-backend-vulkan` crate = **~55% (Wave 7 で +20pp)** = 生 FFI dlopen loader / probe.rs compute queue family selection / 12-entry SPIR-V manifest + SHA-256 pin / GemmPipelineVariant + GemmPipelinePreference + capability-driven pipeline dispatch / BackendKind::Vulkan / Compute seam Vulkan arm foundation slice / **Wave 7 で: VkDevice + vkCreateDevice + ~30 device-level fn pointer resolution (T08) / VulkanCommandPool + reset flag + primary VkCommandBuffer (T09) / VulkanDescriptorSetLayout + Pool + Set + vkUpdateDescriptorSets (T10) / VulkanPipelineLayout + VulkanShaderModule (empty-blob rejection) + VulkanComputePipeline scaffold (T11) / VulkanBuffer + VkDeviceMemory + find_memory_type walking VkPhysicalDeviceMemoryProperties + write/read_bytes via vkMapMemory (T12) / upload_bytes/download_bytes via transient staging buffer + vkCmdCopyBuffer + fence submit (T25) / vkEnumerateDeviceExtensionProperties real coop-matrix walk = VK_KHR_cooperative_matrix / VK_NV_cooperative_matrix (T30)** / smoke_test_runtime_object_stack が backend init で走行 / Drop order correct / Android arm64 cross-build clean、opt-in feature default OFF、~45% 残 = T13-T22 SPIR-V blob 本体 (glslc developer-side) / T23/T24 Backend::supports + eval_op per op / T26-T29 graph-executor Vulkan arm / T32-T34 Whisper base parity on lavapipe / T36-T38 CI wiring / T39-T40 Android 実機 RTF）| § 1（Android 実機 RTF、M3-18 と連動）|
...
| M3-10 | Voxtral 統合 | ✅ **Wave 5 + Wave 6 + Wave 7 追撃**（`089b9c3` + `b1d7aaa` + `c5e0579`、**~80% (Wave 7 で +20pp)** = Whisper 派生 audio encoder + Mistral text decoder + ASR/S2S heads + converter + tokenizer + streaming (Wave 5-6) + **Wave 7 で: TextDecoderSession + KvCache + forward_step (pre-norm RMSNorm → GQA self-attn with RoPE + causal mask + KV cache append → SwiGLU FFN → tied logits) Compute seam 経由 backend 共有、VoxtralAsr::transcribe = whisper::mel::log_mel + AsrHead greedy loop + VoxtralTokenizer::decode で完全接続、Transcription 200 response、vokra-server の Voxtral 経路は 501 → 200 化、push_chunk_pcm + finalize_transcript による multi-chunk accumulate 経路 + offline transcribe との bit-exact e2e smoke**、**honest limitation として明記**: audio conditioning (audio adapter / soft prefix) は現 GGUF converter が weights を書き出していないため未 wire、decoder は BOS token の LM prior continuation = real ASR 品質は adapter follow-up + T19+ real-checkpoint parity に申し送り (silent fallback 禁止 FR-EX-08)、~20% 残 = audio adapter GGUF converter 書き出し + runtime forward 配線 / Metal/CUDA VoxtralDecodeSession (allow_device_session=true path、GPU seam ticket) / beam search / 実 multilang WER 実測 (T22、real HF weight + real audio = 依頼者)）| § 3（audit）|
...
**Wave 1〜Wave 7 完了サマリ（2026-07-09）**: **19 WP 中 16 WP CC 実装コミット完了 + 3 partial WP を Wave 7 で追撃**（Wave 1 = 4 WP / Wave 2 = 3 WP / Wave 3 = 2 WP / Wave 3.5 = 3 WP / Wave 4 = 1 WP / Wave 5 = 3 WP scaffold / Wave 6 = M3-12 新規完成 + M3-04 GPU 完成 + M3-02/09/10 partial 進捗 / **Wave 7 = M3-02 Vulkan runtime object stack + M3-09 LLM backbone scaffold + M3-10 autoregressive greedy decode 完成**）。branch `feat/m3-plan-and-wave1`、main から **25 commits ahead / +27.6k lines**。**Wave 6 は ultracode parallel workflow (`wf_25308de8-a59`、5 agents 32 分、1.36M tokens)**、**Wave 7 は ultracode parallel workflow (`wf_a0a8749a-3e0`、3 agents 31 分、917K tokens)**、いずれも worktree 隔離で並列実装 + verify + commit、統合時 cherry-pick conflict 0。**verify 全 green**: cargo build clean（全 12 crate + vokra-backend-vulkan opt-in default OFF、default / metal / cuda / vulkan feature 全 build 成功、**Android arm64 cross-build for vulkan clean**）／cargo test 全体 = **1542 passed / 0 failed / 4 ignored**（Wave 6 baseline 1490 から **+52 tests**、Wave 5 baseline 1386 から累積 **+156 tests**）／cargo fmt --check clean／cargo clippy `-D warnings` clean／`scripts/check-zero-deps.sh` OK（root Cargo.lock は `vokra-*` のみ、NFR-DS-02 保存）／`scripts/check-abi-changelog.sh` OK（Wave 6/7 は C ABI 変更なし）。**残 CC WP** = **なし** (Wave 6 で M3-12 完成 + Wave 7 で M3-02/09/10 の CC-implementable 分を進捗)。**残依頼者専任 WP** = **M3-18** + **M3-19** + **M3-12-T14 実機 sanity run**。**Partial 実装 WP の依頼者引き渡し分**（follow-up、blocking ではない）= M3-02 ~45% 残 (T13-T22 SPIR-V blob developer glslc + T32-T34 lavapipe parity + T39-T40 Android 実機 owner) / M3-09 ~28% 残 (T02 tensor manifest 提供後 T07/T08 forward + T21-T24 real HF checkpoint parity owner) / M3-10 ~20% 残 (audio adapter GGUF converter + Metal/CUDA VoxtralDecodeSession + beam search + T22 real multilang WER owner) / M3-01 RTF gate（M2-14 defer）。

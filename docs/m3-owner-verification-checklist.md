...
**CC-side status**（2026-07-09 更新、branch `feat/m3-plan-and-wave1`、**Wave 1〜Wave 10 の CC 実装分完了 = 19 WP 中 16 コミット済 + partial WP を Wave 7/8/9/10 で追撃**）:

**ticket spec**: `docs/tickets/m3/` (gitignore) に 19 WP 全 file (M3-01〜M3-19) + README 完備、~340 tickets / 285h の内訳確定 (ultracode workflow 経由)。

**実装コミット (16 WP)** — feat/m3-plan-and-wave1 で main から **42 commits ahead**、**+50,584 lines**:
...
**Verify (2026-07-09、Wave 10 merge 後)**: cargo build clean (全 12 crate + vokra-backend-vulkan opt-in feature、default / metal / cuda / vulkan feature 全 build 成功) / cargo test 全体 **1734 passed / 0 failed / 4 ignored** (default) / **1757 passed** (all features `vulkan,metal,cuda`) + **vokra-server 161 tests** (excluded workspace、Wave 9 baseline 153 から +8) (Wave 9 baseline 1712/1720 から **+22 / +37**、Wave 8 baseline 1585 から累積 **+149 / +172**、Wave 5 baseline 1386 から累積 **+348 / +371**) / cargo fmt --check clean / cargo clippy `-D warnings` clean / `scripts/check-zero-deps.sh` OK (root Cargo.lock は vokra-* only、NFR-DS-02 保存) / `scripts/check-abi-changelog.sh` OK (Wave 10 の M3-16 informational section 追加は script gate 対象外、Wave 9/10 通じて C ABI 変更なし)。

**Partial 実装 WP の残 ticket** (各 WP の spec file を参照して follow-up、blocking ではない):
- **M3-02 Vulkan** (~28% 残、Wave 10 で ~30% → ~28%): T14-T22 実 kernel の glslc 生成 + `kernels/precompiled/*.spv` commit (GEMM 2 variants / GEMV / softmax + softmax_causal / layer_norm / gelu / conv1d / elementwise / activation / shape ops、`scripts/compile-vulkan-shaders.sh` 実行 by developer with `brew install glslang` or Vulkan SDK) / eval.rs の Copy/Add 以外 op arm の SPIR-V dispatch (Wave 9 で Copy+Add は完了、Wave 10 で ADR + CI wiring 完了) / T27-T29 graph-executor 他 op Vulkan arm / T33/T34 Whisper base parity harness (Wave 10 で `gpu-vulkan-parity.yml` scaffold 済、初回 workflow_dispatch は owner) / T37 Android arm64-v8a real-device runner / T39-T40 Android 実機 RTF (M3-18 と併走)
...
- **M3-10 Voxtral** (~2% 残、Wave 10 で ~3% → ~2%): (a) real Voxtral safetensors inspection → 実 adapter tensor 名 + dim + activation 抽出 → `adapter.json` side-car 作成 → `vokra-cli convert --adapter-config` 実行 → 生成 GGUF の loadable 確認 = **依頼者**、(b) `VoxtralAsr::transcribe` の real ASR WER 実測 (T19/T20 fixtures 対 upstream reference) = **依頼者**、(c) 実 multilang WER 実測 (T22、real HF weight + real audio dataset = 依頼者)、(d) **Wave 10.1 / M4**: full device-residency の per-step 1 command buffer 化 = 実 fused Metal MSL / CUDA PTX kernel (Mistral RMSNorm + RoPE + GQA-attn + SwiGLU の融合実装) = M4 kernel fusion チケット (Wave 10 で API surface + ResidencyMode enum + tests + plumbing seam は完了、Wave 9 の thin Compute-seam wrapper は per-op GPU dispatch で機能的に完成)
...
## Summary 進捗表（2026-07-09 更新、Wave 1〜Wave 10 land 完了時点、branch `feat/m3-plan-and-wave1`）
...
**Wave 1〜Wave 10 完了サマリ（2026-07-09）**: **19 WP 中 16 WP CC 実装コミット完了 + partial WP を Wave 6/7/8/9/10 で追撃**（Wave 1 = 4 WP / Wave 2 = 3 WP / Wave 3 = 2 WP / Wave 3.5 = 3 WP / Wave 4 = 1 WP / Wave 5 = 3 WP scaffold / Wave 6 = M3-12 新規完成 + M3-04 GPU 完成 + M3-02/09/10 partial 進捗 / Wave 7 = M3-02 Vulkan runtime object stack + M3-09 LLM backbone scaffold + M3-10 autoregressive greedy decode 完成 / Wave 8 = M3-10 audio adapter framework (real ASR 昇格) + M3-02 SPIR-V blob 生成方針確定 + hand-crafted smoke kernel / Wave 9 = M3-02 add_f32 handcrafted kernel + eval_op(Copy/Add) + graph-executor Vulkan arm + M3-09 LLM forward body + M3-10 beam search + M3-10 GPU decode session (Metal+CUDA) / **Wave 10 = investigation で判明した 7 タスク bundle = correctness bug (transcribe_beam が allow_device_session を honor しない件) + M3-15 server 側 Voxtral beam surface + integration coverage / M3-02 ADR reconstruction + Vulkan parity CI wiring + M3-16 GGUF metadata section / M3-10 full device-residency API surface + ResidencyMode enum**）。branch `feat/m3-plan-and-wave1`、main から **42 commits ahead / +50.6k lines**。**Wave 6/7/8/9/10 いずれも ultracode parallel workflow (worktree 隔離)**: Wave 6 = `wf_25308de8-a59` (5 agents 32 分、1.36M tokens、conflict 0) / Wave 7 = `wf_a0a8749a-3e0` (3 agents 31 分、917K tokens、conflict 0) / Wave 8 = `wf_6016f4e1-172` (2 agents 31 分、610K tokens、conflict 0) / Wave 9 = `wf_05152309-fec` (4 agents 30.6 分、1.15M tokens、voxtral/asr.rs + voxtral/mod.rs 軽微 conflict) / **Wave 10 = `wf_b7fc9cf0-306` (3 agents bundle 31.7 分、789K tokens、voxtral/asr.rs で軽微 conflict = A + C agent 並列 test module 重複、structural resolution で全 test 両立を実現)**。**verify 全 green**: cargo build clean（全 12 crate + vokra-backend-vulkan opt-in default OFF、default / metal / cuda / vulkan feature 全 build 成功）／cargo test 全体 = **1734 passed / 0 failed / 4 ignored** (default) / **1757 passed** (all features `vulkan,metal,cuda`) + **vokra-server 161 tests** (Wave 9 baseline 153 から +8)（Wave 9 baseline 1712/1720 から **+22 / +37**、Wave 8 baseline 1585 から累積 **+149 / +172**、Wave 5 baseline 1386 から累積 **+348 / +371**）／cargo fmt --check clean／cargo clippy `-D warnings` clean／`scripts/check-zero-deps.sh` OK（root Cargo.lock は `vokra-*` のみ、NFR-DS-02 保存）／`scripts/check-abi-changelog.sh` OK（Wave 10 の M3-16 informational section 追加は script gate 対象外、Wave 9/10 通じて C ABI 変更なし）。**残 CC WP** = **なし** (Wave 6 で M3-12 完成 + Wave 7/8/9/10 で M3-02/09/10 の CC-implementable 分を継続進捗)。**残依頼者専任 WP** = **M3-18** + **M3-19** + **M3-12-T14 実機 sanity run** + **M3-10 real adapter.json side-car 作成 + WER 実測**。**Partial 実装 WP の依頼者引き渡し分**（follow-up、blocking ではない）= M3-02 ~28% 残 (T14-T22 実 kernel glslc 生成 + T33/T34 lavapipe parity + T39-T40 Android 実機 owner、Wave 10 で ADR + CI wiring 完了) / M3-09 ~15% 残 (T02 real HF tensor manifest 提供後 `LlmWeights::from_gguf` real weights 対応 + T21-T24 real HF checkpoint parity owner、LLM forward body は Wave 9 で完了) / M3-10 ~2% 残 (real Voxtral safetensors → adapter.json + WER 実測 owner、full device-residency は Wave 10 で API surface + plumbing 完了、実 per-step 1 command buffer kernel は Wave 10.1 / M4 kernel fusion follow-up) / M3-01 RTF gate（M2-14 defer）。

...
| `vokra-ops` <!-- anchor: crates/vokra-ops/src/lib.rs --> | 音声オペレータ。STFT / iSTFT / mel filterbank / MFCC / DCT ほか audio dialect（`FR-OP-*`）と、CPU 側 FFT lowering — C ライブラリの binding ではなく pocketfft アルゴリズムの Rust 再実装。特定モデルではなくここに置かれる共有プリミティブ: 自己教師ありエンコーダ binder 群が共有する ViT 音響エンコーダ（2-D patch embedding + pre-norm Transformer）、WPE 残響除去、YIN / PyIN、逆テキスト正規化、Snake / anti-aliased upsample のボコーダ部品。 |
...
| `vokra-backend-coreml` <!-- anchor: crates/vokra-backend-coreml/src/lib.rs --> | Apple ANE の whole-submodel delegate（M5-01、macOS / iOS、opt-in `coreml` feature）。生 Objective-C runtime + CoreML FFI で binding crate は使わない。hash bind した `.mlmodelc` sidecar が `DelegateBackend` 経由で Whisper encoder 全体を実行し、per-op の `Backend` coverage は設計上空のまま。2026-08-24 M1 bakeoff は配置率 PASS、parity と 2 倍速は FAIL のため C selector は公開しない。 |

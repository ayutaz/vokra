...
| `vokra-cli` <!-- anchor: crates/vokra-cli/src/main.rs --> | 総合コマンドラインツール（`FR-TL-02`）。`run` / `convert` / `bench` / `f0`。**binary crate** であり、`src/lib.rs` を持たない唯一の crate。引数パースは手書き。`f0` が `run --task` ではなく独立 subcommand なのは、YIN / PyIN が weight を持たず、呼び出し側が渡せる `--model` が存在しないため。 |

...
- [ ] Family 5 (Qwen3-TTS, `VOKRA_QWEN3_TTS_ENABLE`): VAST converted the pinned 0.6B release (`5d839924…`) to a 478-tensor, 1,829,328,672-byte GGUF. Its targeted harness passed 12 tests and matched the upstream talker (13 axes) and code-predictor (10 axes) config exactly. The separately released pinned 12 Hz tokenizer now has a strict 271-tensor decode-only GGUF contract and a native mapped CPU/Metal waveform-decoder graph. Keep open because real-weight PCM parity for that decoder and native end-to-end talker/code-predictor synthesis remain absent, and repository scheduling is undecided.

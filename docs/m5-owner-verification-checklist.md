...
- [ ] Family 5 (Qwen3-TTS, `VOKRA_QWEN3_TTS_ENABLE`): VAST converted the pinned 0.6B release (`5d839924…`) to a 478-tensor, 1,829,328,672-byte GGUF. Its targeted harness passed 12 tests and matched the upstream talker (13 axes) and code-predictor (10 axes) config exactly. Conversion now embeds and authenticates the fixed-revision config, byte-BPE and generation sidecars for all five public main checkpoints, and the runtime implements the official prompt/language/fixed-speaker token boundary. The separately released pinned 12 Hz tokenizer has a strict 271-tensor decode-only GGUF contract and a native mapped CPU/Metal waveform-decoder graph. Keep open because the mapped autoregressive talker, 15-row code-predictor loop, two-artifact CLI join and real-weight parity remain absent; repository scheduling is also undecided.

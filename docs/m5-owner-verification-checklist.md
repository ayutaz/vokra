...
**2026-08-18 current-state rule**: the earlier **94 unchecked boxes** were a historical owner ledger, **not** a count of 94 missing implementations. The 2026-08-17 reconciliation found 41 live action-ledger boxes; the repeatable SBV2 SDP VAST gate and both misaki environment rows closed on 2026-08-18, leaving **38 unchecked boxes**. This remains an action ledger rather than an implementation metric: a box can mean an external legal/infra decision, real-weight access, a deliberately fail-closed policy, a future backend, or a partially landed implementation that still lacks its real-checkpoint proof. Only mark a box complete when its literal done-condition is evidenced; do not infer implementation status from the unchecked total. The reconciliation below removes stale merge/sign-off/implementation claims while retaining genuine follow-up work.
...
- [ ] Family 8 (deepfilternet3, `VOKRA_DFN3_ENABLE`): HF-card read (Rikorose/DeepFilterNet MIT/Apache-2.0 dual; §3.1 Commercial decision recorded) → set `VOKRA_DFN3_ENABLE=1` → `gh workflow run parity-deepfilternet3-real.yml -f force_parity=true` → PASS verdict confirmed. The old `VOKRA_DFN3_DATA_URL` blocker is closed: the workflow now creates the independent upstream bundle from a checked-in uv lock. VAST `47955178` proved the exact path on 2026-08-18 with all 21 stage/output bounds green (`enhanced` max |Δ| `4.172e-7`, upstream/Vokra SI-SNR both `14.768 dB`, no tolerance change). This row remains open only for the repository variable + first GitHub Actions dispatch/confirmed run; local `gh` authentication is currently invalid.
- [ ] Family 9 (deberta-v3-large, `VOKRA_DEBERTA_V3_ENABLE`): HF-card read (microsoft/deberta-v3-large MIT; §3.1 Commercial decision recorded 2026-07-27) → set `VOKRA_DEBERTA_V3_ENABLE=1` → `gh workflow run parity-deberta-v3-large-real.yml` → PASS verdict confirmed. The old “no Rust consumer” description was stale: `crates/vokra-bert/tests/deberta_v3_real.rs` landed on 2026-07-29 and consumes the upstream `input_ids` + `final_hidden` dump. The workflow now uses a dedicated Linux-x86_64 uv lock, removes the obsolete `VOKRA_DEBERTA_V3_HARNESS_READY` gate, and defaults matching PRs / enabled schedules / manual dispatches to the real final-hidden numerical leg. VAST `47955178` proved the full path on 2026-08-18: converter real smoke passed, tokenizer loaded 128,000 pieces, and final-hidden max |Δ| was `1.049042e-5` under the unchanged `6.0e-3` bound after fixing v3 relative-embedding LayerNorm and exact GELU. This row remains open only for the repository variable + first GitHub Actions dispatch/confirmed verdict; local `gh` authentication is currently invalid. Per-layer hidden/attention taps remain a separately disclosed extension, not a missing final-output consumer. See `docs/handoff/parity-deberta-v3-large-real.md`.

- [ ] Family 10 (SBV2, sidecar-hash gate): `parity-sbv2-real.yml` first validates the three real sidecars for the current main + JA BERT + EN BERT numerical path, then runs the real dump and the named non-ignored `parity_sbv2_real` consumer. VAST `47955178` proved this full path on 2026-08-18 with all intermediate/duration/waveform/mel-loss assertions green (waveform max |Δ| `4.521498e-2`, mel-loss RMS `1.655322e-1`, no tolerance change). The VAST run also exposed and fixed four stale workflow details: ad-hoc pip, floating revisions, absent regenerated-hash verification, and `--ignored` filtering out the consumer after WP-06 removed its ignore attribute. This row remains open only for the first corrected GitHub Actions dispatch/confirmed verdict; local `gh` authentication is currently invalid. The published ZH BERT sidecar proves the separate WP-19 four-file loader input is available; it is **not** a ZH numerical-parity PASS because the ZH reference dumper/harness does not exist yet. See `docs/handoff/parity-sbv2-real-vast-2026-08-18.md` and `docs/handoff/parity-ci-flip-switch.md`.
...
- [ ] For SBV2, the three current numerical-path inputs and JA/EN dump + `parity_sbv2_real` are VAST-proven as of 2026-08-18. The remaining part of this combined row is the missing ZH reference-dump/harness; do not claim four-file numerical parity before it lands.
...
- [x] **openbmb/VoxCPM2-2B** (license-audit row 296) — Apache-2.0 signed
  ...
  checked implementation row does not change the 38-box action-ledger total.

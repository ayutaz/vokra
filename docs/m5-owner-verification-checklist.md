...
**2026-08-18 current-state rule**: the earlier **94 unchecked boxes** were a historical owner ledger, **not** a count of 94 missing implementations. The 2026-08-17 reconciliation found 41 live action-ledger boxes; the repeatable SBV2 SDP VAST gate closed on 2026-08-18, leaving **40 unchecked boxes**. This remains an action ledger rather than an implementation metric: a box can mean an external legal/infra decision, real-weight access, a deliberately fail-closed policy, a future backend, or a partially landed implementation that still lacks its real-checkpoint proof. Only mark a box complete when its literal done-condition is evidenced; do not infer implementation status from the unchecked total. The reconciliation below removes stale merge/sign-off/implementation claims while retaining genuine follow-up work.
...
- [ ] **Voxtral-Small-24B-2507** (row 251) — Apache-2.0 signed 2026-07-23 yousan. **48-GB real conversion + publish dry-run completed on vast.ai 2026-08-18** from pinned upstream commit `da5b42409f279fdd92febee0511a6c32828569c1` (11 shards only; duplicate `consolidated.safetensors` excluded): 852 tensors / 851 exact BF16 passthrough / 0 skipped / tokenizer embedded / 48,542,408,640 bytes / SHA-256 `52f8607972c00ddc2a9a5736c2eaf2562878edafaa7a56e1de053ceac9a6109f`. Header gate confirms `name=voxtral-small-24b` and source `mistralai/Voxtral-Small-24B-2507 (Apache-2.0)`; §3.1, model-card, LICENSE, NOTICE, SOURCE and all dry-run publish gates pass. The existing live HF artifact at `vokra/voxtral-small-24b-2507` is 24B-sized but carries stale false Mini-3B provenance, so it is not accepted as completion and must be replaced by this corrected artifact. Instance `47955178` is stopped with the staged artifact preserved pending explicit authorization to transfer the HF credential to VAST and run `publish-one.sh --push`; never move the artifact or upload work to the M1 iMac. Remaining literal done-conditions: corrected-artifact upload/live verification, then real ASR/runtime parity. This box therefore remains open and the action-ledger total remains 40.

...
> **選定・契約・費用判断は一切 CC で行わない (owner 領分、T12)**。CC は要件 checklist と候補比較の枠のみ用意し、候補の性能・価格を出典なしに記入しない（現行の作業規律は `AGENTS.md` と関連 skill）。
...
|---|------|-----------|------|-----------|
| R1 | **cross-origin isolation ヘッダ (COOP/COEP) の設定可否** | WASM Threads は `SharedArrayBuffer` + COOP (`Cross-Origin-Opener-Policy: same-origin`) + COEP (`Cross-Origin-Embedder-Policy: require-corp`) が前提。**配信側でレスポンスヘッダを制御できない CDN/ホスティングでは Threads leg が成立しない** (single-thread fallback に縮退)。 | `docs/platform-support/v1.0-rc-support-matrix.md` / milestones §8 M4-01 対応 ISA 列 | ☐ 可 / ☐ 不可 |
...
| R3 | **大容量 GGUF モデルファイルの Range request / キャッシュ / 転送コスト** | GGUF は mmap 前提の大容量 asset。ブラウザ配信では帯域コストが Web デモ運用費の支配項になる見込み。Range request 対応の可否・キャッシュ TTL・egress 課金体系が比較観点。**実測系の数値はここに書かない** (owner が候補ごとに確認)。 | `docs/design/vokra-gguf-chunks.md` / deliverables §3.2 Web 行 | ☐ 確認 |
...
| R5 | **`vokra.dev` / `vokra.io` / `vokra.ai` ドメインとの接続性** | Web デモ・npm 配信の canonical ドメイン。未取得ならその旨を honest 記録し、取得は owner backlog。 | milestones §11 先行タスク「ドメイン取得」行（旧設計資料は履歴） | ☐ 取得済 / ☐ 未取得 |

...
| **openbmb/VoxCPM2** | Apache-2.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 4 tts-continuous-vae family（parity-tts-continuous-vae-real.yml、pins.yaml `voxcpm2`）。HF card license = apache-2.0。**注**: openbmb/VoxCPM2 は 2B params 30-lang model = 別モデル。現在の Vokra converter `voxcpm2` kind は `openbmb/VoxCPM-0.5B`（下記別 row）用に構築されており、本 2B model 対応は未実装 = converter 拡張が済むまで sign-off 保留。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |

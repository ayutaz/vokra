...
| **kyutai/stt-2.6b-en** | CC-BY-4.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 2 の NeMo-ASR family（parity-nemo-asr-real.yml、pins.yaml `kyutai-stt-2.6b-en`）。HF card license = cc-by-4.0（要 attribution）。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **nvidia/parakeet-tdt-0.6b-v3** | CC-BY-4.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 2 NeMo-ASR family（pins.yaml `parakeet-tdt-0.6b-v3`）。HF card license = cc-by-4.0。NVIDIA release ゆえ NVIDIA-EULA との重畳判定は owner。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **nvidia/parakeet-ctc-1.1b** | CC-BY-4.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 2 NeMo-ASR family（pins.yaml `parakeet-ctc-1.1b`）。HF card license = cc-by-4.0。NVIDIA-EULA 重畳判定は owner。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |

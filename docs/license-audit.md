...
| **ResembleAI/chatterbox (T3 mtl23ls_v3)** | MIT | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 tts-hiftnet family（pins.yaml `chatterbox-mtl23ls-v3`）。HF card license = mit。Chatterbox-Multilingual T3 バリアント。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **ResembleAI/chatterbox-turbo** | MIT | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 tts-hiftnet family（pins.yaml `chatterbox-turbo-v1`）。HF card license = mit。t3_turbo_v1 バリアント。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
...
| **deberta-v2-large-japanese-char-wwm** (`ku-nlp/deberta-v2-large-japanese-char-wwm`) | cc-by-sa-4.0 | 2026-07-26（Task 30、HF Hub API で `config.json` / model card を直接確認） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SBV2 v2 が要求する JA BERT エンコーダ（`crates/vokra-bert/src/deberta_v2.rs`、`crates/vokra-convert/src/models/deberta_v2.rs`、`checkpoint.bert_ja`）。一次資料 = HF Hub API 経由で `ku-nlp/deberta-v2-large-japanese-char-wwm` の `config.json` / model card を Task 30 が直接 fetch（`tools/parity/deberta_v2_dump_reference.py` module doc 記載）。`LicenseClass::from_license_str("cc-by-sa-4.0")` → `Copyleft`（ShareAlike variant。商用可だが派生物（Vokra が生成する GGUF）にも同ライセンスが継承され、attribution も必須）。T3 Copyleft 配布経路は実装済みだが、実 checkpoint の in-hand 検証が未完了（`tests/fixtures/sbv2/README.md` 明記）。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |

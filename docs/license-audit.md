...
| **distil-whisper/distil-large-v3.5** | MIT | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 whisper-extras family（parity-whisper-extras-real.yml、pins.yaml `distil-large-v3.5`）。HF card license = mit。Whisper large-v3 quintuple + 2-layer decoder。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **kotoba-tech/kotoba-whisper-v2.2** | Apache-2.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 whisper-extras family（pins.yaml `kotoba-whisper-v2.2`）。HF card license = apache-2.0。JA distillation variant。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **nari-labs/Dia-1.6B** | Apache-2.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 tts-dac family（parity-tts-dac-real.yml、pins.yaml `dia-1.6b`）。HF card license = apache-2.0、DAC 44.1 kHz terminal codec。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
...
| **Qwen/Qwen3-TTS-12Hz-0.6B-Base** | Apache-2.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 3 qwen3-tts family（parity-qwen3-tts-real.yml、pins.yaml `qwen3-tts-12hz-0.6b-base`）。HF card license = apache-2.0。1.7B variant 未公開ゆえ本 tier では唯一の family member。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **openbmb/VoxCPM2** | Apache-2.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 4 tts-continuous-vae family（parity-tts-continuous-vae-real.yml、pins.yaml `voxcpm2`）。HF card license = apache-2.0。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **microsoft/VibeVoice-1.5B** | MIT | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 4 tts-continuous-vae family（pins.yaml `vibevoice-1.5b`）。HF card license = mit。Microsoft release。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
| **Aratako/Irodori-TTS-500M-v3** | MIT | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase JA tts-japanese family（parity-tts-japanese-real.yml、pins.yaml `irodori-tts-500m-v3`）。HF card license = mit。Rectified-Flow DiT + Semantic-DACVAE-JA 48 kHz。**HONEST SLUG CORRECTION**: SoTA plan は `Irodori-tech/…` を列挙していたが HTTP 401、実 public repo は `Aratako/…`（workflow header 参照）。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |

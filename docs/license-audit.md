...
| **FunAudioLLM/Fun-CosyVoice3-0.5B-2512** | Apache-2.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 tts-hiftnet family（parity-tts-hiftnet-real.yml、pins.yaml `fun-cosyvoice3-0.5b-2512`）。HF card license = apache-2.0。HiFTNet terminal vocoder（NSF + iSTFTNet + Snake）。torch pickles → safetensors sidecar が Phase 2 owner task。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |

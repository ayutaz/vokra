...
| **Zyphra/Zonos-v0.1-transformer** | Apache-2.0 | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 tts-dac family（pins.yaml `zonos-v0.1-transformer`）。HF card license = apache-2.0、DAC 44.1 kHz terminal codec。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
...
| **ResembleAI/chatterbox-nano** | MIT | 2026-07-25（family harness pin） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SoTA plan Phase 1 tts-hiftnet family（pins.yaml `chatterbox-nano-v1`）。HF card license = mit。t3_nano_v1 = family 最小。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |
...
| **deberta-v3-large** (`microsoft/deberta-v3-large`) | MIT | 2026-07-26（Task 30、HF Hub API で `config.json` / model card を直接確認） | ______________ | ☐ Commercial / ☐ Research-only / ☐ Rejected | **未判定**: SBV2 v2 が要求する EN BERT エンコーダ（`crates/vokra-bert/src/deberta_v3.rs`、`crates/vokra-convert/src/models/deberta_v3.rs`、`checkpoint.bert_en`）。一次資料 = HF Hub API 経由で `microsoft/deberta-v3-large` の `config.json` / model card を Task 30 が直接 fetch。`LicenseClass::from_license_str("mit")` → `Permissive`（商用利用・再配布とも許諾、追加条件なし）。ライセンス自体は他の Permissive 行と同様に単純だが、SBV2 v2 は 3 チェックポイント（sbv2 主体 + JA/EN BERT）を一体で配布する構成であり、実 checkpoint の in-hand 検証が 3 行とも未完了のため本行も揃えて sign-off 保留（`tests/fixtures/sbv2/README.md` 明記）。**本欄の署名・判定は owner 記入、CC は pre-fill しない**。本行 sign-off まで公式配布不可（fail-closed） |

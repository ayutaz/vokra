...
> **2026-09-09 audit-start boundary:** This dated scope-decision record is
> historical and does not describe the live Mac CPU/Metal campaign. Before
> this documentation refresh, PR #79 was at `9efcd16e` (`CLEAN` / `MERGEABLE`;
> 110 CI success / 13 intentional skip / 0 fail), exact VAST implementation
> evidence was `caf70eb1` (305 suites / 8,010 passed / 0 failed / 100 ignored),
> VAST `50320338` plus 150-GB storage were destroyed, and Scaleway had not
> started. Current 63-row status is in the canonical pre-Scaleway ledger.
...
- **FA v3 前倒し禁止**（設計制約 §5-(7)、既に M4-07 内スコープ）。Hopper WGMMA/TMA 特化ゆえ Ampere/Ada 顧客層で恩恵薄、旧 v1.5+（現 M4/v1.0-rc）押し出しは Kill switch 判断ではなく設計制約
...
- **M4-07 FA v3 の M4 内早期実装**: 既に M4 スコープ内、旧 v1.5+（現 M4/v1.0-rc）前倒し禁止の設計制約 §5-(7) は Ampere/Ada 支配的な顧客層根拠で不変

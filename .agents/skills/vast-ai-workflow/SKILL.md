...
**強制されている**: `.codex/hooks/guard-local-memory.sh` が PreToolUse フックとして上記を**ブロック**する（`.codex/hooks.json` に登録済み、`--self-test` 24 ケース）。`.githooks/pre-push` も maintainer Mac の deep Cargo path を開始前に拒否する。意図的に通す場合は、その1回を依頼者が明示承認したときだけ `VOKRA_ALLOW_LOCAL_HEAVY=1` を前置する。

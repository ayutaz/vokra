// VokraStream.cs — high-level wrapper over a Vokra VAD stream (M0-10-T04).
//
// A stateful Silero VAD stream: push mono f32 PCM, drain per-frame speech
// probabilities. All recurrent state (LSTM h/c, framing) is hidden in the native
// handle (FR-LD-06). Call from the single worker thread (T07).

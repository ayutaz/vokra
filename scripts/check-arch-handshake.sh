...
#   citations on `mossformer2_ss_16k` and `openwakeword`). Entries that remain
#   gaps carry a dated CORRECTION; MossFormer2 later left the ledger entirely
#   when its strict native reader landed. Two failure modes recurred and are
#   worth naming:
...
declare -a NO_CONVERTER=(
  'sortformer|binder arch exists, but the Sortformer-Diar-4spk-v1 converter is currently INSPECTION_ONLY: its VAST-authenticated identity, source/config/tensor evidence, and license are unresolved, so it intentionally emits no artifact or arch stamp. Keep the binder-only fact explicit until the fixed source and weight contract is authenticated.'
)
...
declare -a NO_STAMP=(
  ...
  'canary_qwen|the converter rejects arbitrary or composite input fail-closed because its required model/source metadata is not authenticated. It cannot stamp the required metadata until the fixed composite contract is authenticated on VAST; no guessed or locally invented stamp is permitted.'
  'cosyvoice2|the converter rejects arbitrary or composite input fail-closed because its required model/source metadata is not authenticated. It cannot stamp the required metadata until the fixed composite contract is authenticated on VAST; no guessed or locally invented stamp is permitted.'
  'csm|the converter rejects arbitrary or composite input fail-closed because its required model/source metadata is not authenticated. It cannot stamp the required metadata until the fixed composite contract is authenticated on VAST; no guessed or locally invented stamp is permitted.'
  'irodori|the converter rejects arbitrary or composite input fail-closed because its required model/source metadata is not authenticated. It cannot stamp the required metadata until the fixed composite contract is authenticated on VAST; no guessed or locally invented stamp is permitted.'
  'kyutai_stt|the converter rejects arbitrary or composite input fail-closed because its required model/source metadata is not authenticated. It cannot stamp the required metadata until the fixed composite contract is authenticated on VAST; no guessed or locally invented stamp is permitted.'
  'sortformer|the converter rejects arbitrary or composite input fail-closed because its required model/source metadata is not authenticated. It cannot stamp the required metadata until the fixed composite contract is authenticated on VAST; no guessed or locally invented stamp is permitted.'
  'sgmse|the SGMSE-VoiceBank converter is intentionally inspection-only and rejects every conversion with AUTHENTICATED_MANIFEST_REQUIRED (crates/vokra-convert/src/models/sgmse.rs:147-150). The native binder cannot accept an artifact while REVIEWED_TENSOR_MANIFEST_SHA256 and REVIEWED_TENSOR_ROLES remain None (crates/vokra-models/src/sgmse/mod.rs:47-50, 455-471), so stamping vokra.sgmse.tensor_manifest or vokra.sgmse.tensor_manifest_sha256 from the recovered but unreviewed 647-tensor manifest would be an unauthenticated unlock. VAST evidence still leaves typed role assignment, locked distribution integration, independent reference, and EMA selection unresolved; keep the gap explicit until the reviewed manifest and binder constants exist.'
  'owsm_v4_medium_1b|the OWSM v4 medium 1B converter is intentionally INSPECTION_ONLY and rejects every conversion even though VAST has authenticated the fixed source tree, checkpoint container, and 1,172-name/shape/dtype structural inventory. Per-tensor payload mapping/hashes and normalization, the GGUF writer contract, frontend/decoder/tokenizer/CTC-attention execution, independent real-weight parity, and dependency/dataset review remain incomplete; its 24 vokra.owsm_v4_medium_1b metadata/tensor-payload requirements must not be self-stamped to unlock the strict partial binder.'
)

#!/usr/bin/env bash
# VAST-only, no-upload MAGNeT Small validation preflight.
...
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"; PROJECT="$ROOT/tools/parity/magnet_small_10secs"; GATE="$ROOT/tools/parity/audiocraft_safe_gate.py"
HF_REPOSITORY="facebook/magnet-small-10secs"; HF_REVISION="2c9084771bd2e83c5c7e36303e24550da30da8e0"
SOURCE_URL="https://github.com/facebookresearch/audiocraft.git"
SOURCE_REVISION="905371a779f608169353fe6ad42bb5fc10c5c9a8"; README_BYTES=10693; README_SHA256="e7fba2ce044a85fdcff253fa250e661044d9071d6f5033e5eab3f2ca42ce16e4"; COMPRESSION_BYTES=236003715; COMPRESSION_SHA256="91598c7da3d183eb8e0cc19cbbdc4f64f2d0c53069f9c8aa84185d0e33873c67"; STATE_BYTES=840844851; STATE_SHA256="0594e551ed9c40464b5918f5ddcce348e491e912e61d69f4d5d64d4ddd1a6ade"
die(){ echo "magnet-small-10secs validation: BLOCKED: $*" >&2; exit 2; }
...
log(){ echo "magnet-small-10secs validation: $*" >&2; }
...
approval=''; work="${AUDIOCRAFT_WORK_DIR:-/tmp/vokra-audiocraft-work/magnet-small-10secs}"; seen_approval=0; seen_work=0
...
[[ "$(uname -s)" == Linux && "$(uname -m)" == x86_64 ]] || die 'Linux x86_64 VAST required'; [[ "${VOKRA_PUBLISH_ON_VAST:-0}" == 1 ]] || die 'VOKRA_PUBLISH_ON_VAST=1 required'; mkdir -p "$work/model-snapshot" "$work/hf-cache"; uv sync --project "$PROJECT" --frozen --python 3.12; uv run --project "$PROJECT" --frozen --python 3.12 python -c 'import sys; from huggingface_hub import snapshot_download; snapshot_download(repo_id=sys.argv[1],revision=sys.argv[2],local_dir=sys.argv[3],cache_dir=sys.argv[4],local_dir_use_symlinks=False,allow_patterns=["README.md","compression_state_dict.bin","state_dict.bin"])' "$HF_REPOSITORY" "$HF_REVISION" "$work/model-snapshot" "$work/hf-cache"; verify_model_snapshot "$work/model-snapshot"; AUDIOCRAFT_SOURCE_URL="${AUDIOCRAFT_SOURCE_URL:-}"; [[ "$AUDIOCRAFT_SOURCE_URL" == "https://github.com/facebookresearch/audiocraft.git" ]] || die 'AUDIOCRAFT_SOURCE_URL must equal the fixed source origin'; mkdir -p "$work/source"; git -C "$work/source" init; git -C "$work/source" remote add origin "$AUDIOCRAFT_SOURCE_URL"; git -C "$work/source" fetch --depth 1 origin "$SOURCE_REVISION"; git -C "$work/source" checkout --detach FETCH_HEAD; [[ "$(git -C "$work/source" rev-parse HEAD)" == "$SOURCE_REVISION" ]] || die 'source revision mismatch'; verify_source "$work/source"; uv run --project "$PROJECT" --frozen --python 3.12 python "$PROJECT/prepare_checkpoint.py" --input-dir "$work/model-snapshot" --output "$work/magnet-small-10secs.safetensors"; log 'MEASURED_NOT_GATED: authenticated preparation complete; no independent runtime parity is claimed'

...
# This runner never downloads, executes, or publishes a model.
set -euo pipefail
...
self_test() {
  local token
  for token in '--parity' 'Linux x86_64' '32 GiB' 'REFERENCE_READY' 'NO_UPLOAD' 'eager' 'F32' 'MEASURED_PASS' 'atomic' 'no-replace' "$TEST_NAME"; do
...
require_file "$gguf" 'LLM component GGUF'; require_dir "$reference" 'official reference directory'; [[ "$evidence" = /* ]] || die 'parity evidence must be absolute'; [[ ! -e "$evidence" && ! -L "$evidence" ]] || die 'parity evidence must be absent'
for name in manifest.json token-ids.json true_hf_logits.npy; do require_file "$reference/$name" "reference $name"; done
...
log 'running exact official-reference parity test'; CARGO_BUILD_JOBS=1 cargo test -p vokra-models --test parity_cosyvoice2_llm_component_real "$TEST_NAME" -- --ignored --exact >"$log_file" 2>&1 || { tail -n 80 "$log_file" >&2; die 'CosyVoice2 LLM parity failed'; }
...
uv run --no-project --python 3.12 python -c 'import hashlib,json,os,sys; from pathlib import Path; out,gguf,log,manifest,gb,gs,ls,ms=map(Path,sys.argv[1:9]); body={"format":"vokra-cosyvoice2-llm-component-parity-v1","status":"MEASURED_PASS","component":"llm","gguf":{"path":str(gguf),"bytes":int(gb.name),"sha256":gs.name},"reference":{"path":str(manifest.parent),"manifest_sha256":ms.name},"log":{"path":str(log),"sha256":ls.name},"atol":3e-4,"argmax":"ALL_POSITIONS_EXACT","publication":"NO_UPLOAD","model_execution":"NOT_RUN"}; temp=out.parent/("."+out.name+"."+str(os.getpid())+".tmp"); fd=os.open(temp,os.O_WRONLY|os.O_CREAT|os.O_EXCL,0o644); os.write(fd,(json.dumps(body,indent=2,sort_keys=True)+"\\n").encode()); os.fsync(fd); os.close(fd); os.link(temp,out); temp.unlink(); print(json.dumps(body,sort_keys=True))' "$evidence" "$gguf" "$log_file" "$reference/manifest.json" "$gguf_bytes" "$gguf_sha" "$log_sha" "$manifest_sha"
log "MEASURED_PASS recorded atomically at $evidence; NO_UPLOAD; MODEL_EXECUTION_NOT_RUN"

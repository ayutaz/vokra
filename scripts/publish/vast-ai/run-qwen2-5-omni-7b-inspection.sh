...
GATE="tools/parity/qwen2_5_omni_7b_gate.py"
...
self_test() {
  ...
  for required in "$HF_REPOSITORY" "$HF_REVISION" "$SOURCE_REPOSITORY" "$SOURCE_REVISION" "$TRANSFORMERS_TAG" "$TRANSFORMERS_REVISION" "$INSPECTOR" "$GATE" "SHARD_BYTES" "spk_dict.pt" "weights_only=True" "MAX_HEADER_BYTES" "INSPECTION_ONLY" "NO_UPLOAD" "BLOCKED" "git_blob_sha1" "lfs_sha256" "thinker_config" "token2wav_config" "qwen2_5_omni" "local_dir" ".cache" "approval-evidence" "approval-sha256" "expected-head" "--enforce" "BLOCKED_APPROVAL/INSPECTION_ONLY/NO_UPLOAD" "UV_NO_CACHE=1" "--no-project" "--offline"; do
  ...
  for required in 'uname -s' 'uname -m' 'VOKRA_PUBLISH_ON_VAST' 'findmnt' 'git status --porcelain --untracked-files=all' 'snapshot_download' 'model_info' 'list_repo_tree' 'CARGO_BUILD_JOBS' 'qwen2_5_omni_7b_prepare_checkpoint.py'; do
  ...
  if ! UV_NO_CACHE=1 uv run --no-cache --no-project --offline --python 3.12 python "$root/$GATE" --self-test >/dev/null; then echo "self-test FAIL: stdlib blocked gate" >&2; fail=1; fi
  gate_line="$(grep -n -- '^gate_output=' "$self" | head -n1 | cut -d: -f1)"
  host_line="$(grep -n -- 'uname -s' "$self" | tail -n1 | cut -d: -f1)"
  work_line="$(grep -n -- 'mkdir -p "\$work_dir"' "$self" | tail -n1 | cut -d: -f1)"
  snapshot_line="$(grep -n -- 'snapshot_download' "$self" | tail -n1 | cut -d: -f1)"
  inspector_gate_line="$(grep -n -- 'gate.enforce_blocked_approval' "$root/$INSPECTOR" | tail -n1 | cut -d: -f1)"
  preparer_gate_line="$(grep -n -- 'gate.enforce_blocked_approval' "$root/tools/parity/qwen2_5_omni_7b_prepare_checkpoint.py" | tail -n1 | cut -d: -f1)"
  inspector_input_line="$(grep -n -- 'all inspection paths are required' "$root/$INSPECTOR" | head -n1 | cut -d: -f1)"
  preparer_input_line="$(grep -n -- 'input-dir and --output are required' "$root/tools/parity/qwen2_5_omni_7b_prepare_checkpoint.py" | head -n1 | cut -d: -f1)"
  if [[ -z "$gate_line" || -z "$host_line" || -z "$work_line" || -z "$snapshot_line" || -z "$inspector_gate_line" || -z "$preparer_gate_line" || -z "$inspector_input_line" || -z "$preparer_input_line" ]] \
    || (( gate_line >= host_line || gate_line >= work_line || gate_line >= snapshot_line )) \
    || (( inspector_gate_line >= inspector_input_line || preparer_gate_line >= preparer_input_line )); then
    echo "self-test FAIL: blocked gate ordering regression" >&2
    fail=1
  fi
  ...
  if bash "$self" --self-test --approval-evidence /private/tmp/qwen2-omni-approval >/dev/null 2>&1; then echo "self-test FAIL: mixed gate args accepted" >&2; fail=1; else status=$?; [[ "$status" == 2 ]] || { echo "self-test FAIL: mixed gate args exited $status" >&2; fail=1; }; fi
...
approval_evidence=""; approval_sha256=""; expected_head=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --self-test) self=1; shift;;
    --work-dir) [[ $# -ge 2 ]] || die "--work-dir requires path"; work_dir="$2"; shift 2;;
    --approval-evidence) [[ $# -ge 2 && -z "$approval_evidence" ]] || die "--approval-evidence requires one unique path"; approval_evidence="$2"; shift 2;;
    --approval-sha256) [[ $# -ge 2 && -z "$approval_sha256" ]] || die "--approval-sha256 requires one unique value"; approval_sha256="$2"; shift 2;;
    --expected-head) [[ $# -ge 2 && -z "$expected_head" ]] || die "--expected-head requires one unique value"; expected_head="$2"; shift 2;;
    -h|--help) echo "usage: $0 [--work-dir TMPFS] | --self-test"; exit 0;;
    *) die "unknown argument: $1";;
  esac
done
if ((self == 1)); then [[ "$work_dir" == "/dev/shm/vokra-qwen2-5-omni-7b-inspection" && -z "$approval_evidence" && -z "$approval_sha256" && -z "$expected_head" ]] || die "--self-test accepts no other arguments"; self_test; exit $?; fi
...
[[ -f Cargo.toml && -d .git && -f "$GATE" ]] || die "not a Vokra checkout or Qwen gate is missing"
set +e
...
gate_status=$?
set -e
if [[ "$gate_status" != 2 || "$gate_output" != *"BLOCKED_APPROVAL/INSPECTION_ONLY/NO_UPLOAD"* ]]; then die "blocked approval evidence is malformed, stale, or not caller-bound"; fi
die "$gate_output"
...
set +e; "${UV_CMD[@]}" "$INSPECTOR" --snapshot "$snapshot" --source "$source" --transformers "$transformers" --server-tree "$tree" --output "$evidence" \
  --approval-evidence "$approval_evidence" --approval-sha256 "$approval_sha256" --expected-head "$expected_head"; status=$?; set -e

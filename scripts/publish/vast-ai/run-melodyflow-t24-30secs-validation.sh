#!/usr/bin/env bash
# VAST-only, no-upload MelodyFlow validation preflight.
...
die(){ echo "melodyflow-t24-30secs validation: BLOCKED: $*" >&2; exit 2; }
...
log(){ echo "melodyflow-t24-30secs validation: $*" >&2; }
...
validate_work(){ local work="$1" approval="$2" canonical_work approval_dir; [[ ! -e "$work" && ! -L "$work" ]] || die 'work-dir must be absent/nonexistent'; canonical_work="$(canonical_candidate "$work")"; approval_dir="$(cd -P "$(dirname "$approval")" && pwd -P)"; paths_overlap "$canonical_work" "$(canonical_candidate "$ROOT")" && die 'work-dir overlaps checkout'; paths_overlap "$canonical_work" "$approval_dir" && die 'work-dir overlaps approval'; paths_overlap "$canonical_work" "$(canonical_candidate "$PROJECT")" && die 'work-dir overlaps project'; return 0; }

...
source_only(){
  ...
  for command in git uv awk find df findmnt sha256sum readelf; do command -v "$command" >/dev/null || die "missing tool: $command"; done
  ...
  [[ -f "$DEPENDENCY_AUDIT" && ! -L "$DEPENDENCY_AUDIT" ]] || die 'dedicated OWSM dependency audit is missing or symlinked'
  ...
  # Resolve only the pinned reference dependencies on the disposable worker.
  # The audit reads installed metadata and exact locked sdists; it never
  # imports ESPnet/model code or requests a checkpoint.  The only accepted
  # blocked report here is validated below before stopping acquisition.
  uv sync --frozen --project "$REFERENCE_PROJECT" --no-dev >"$work/evidence/reference-project-sync.log" 2>&1 || die 'dedicated OWSM reference dependency sync failed'
  set +e
  ...
  dependency_rc=$?
  set -e
  [[ "$dependency_rc" == 2 ]] || die 'OWSM dependency/license preflight changed unexpectedly'
  set +e
  uv run --frozen --project "$REFERENCE_PROJECT" --python 3.12 python "$DEPENDENCY_AUDIT" --project "$REFERENCE_PROJECT" --output "$work/evidence/dependency-audit.json" --expected-head "$expected_head" >"$work/evidence/dependency-audit.log" 2>&1
  audit_rc=$?
  set -e
  [[ "$audit_rc" == 2 && -f "$work/evidence/dependency-audit.json" ]] || die 'OWSM dependency audit did not produce its intentional blocked report'
  UV_NO_CACHE=1 uv run --no-cache --no-project --offline --python 3.12 python "$DEPENDENCY_AUDIT" --validate-evidence "$work/evidence/dependency-audit.json" --project "$REFERENCE_PROJECT" --expected-head "$expected_head" >"$work/evidence/dependency-evidence-validator.log" 2>&1 || die 'OWSM dependency audit report is not the exact known factual blocker'
  die 'OWSM official reference blocked: known factual blocker primary license bytes unavailable: torch-complex==0.4.4; owner review remains pending; no source/BPE/model acquisition'
...
self_test(){ local path="${BASH_SOURCE[0]}" token fail=0; for token in "$HF_REPOSITORY" "$HF_REVISION" "$SOURCE_URL" "$SOURCE_REVISION" "$STATS_RELATIVE" "$STATS_SHA256" "$CONFIG_GIT_BLOB_SHA1" 'allow_pickle=False' 'weights_only=True' 'canonical_payload_sha256' 'source_name_set_sha256' 'next_vast_command' 'MISSING_OWSM_GGUF_WRITER_CONTRACT' 'structural-manifest' 'resolved revision mismatch' 'selected materialized files mismatch' 'inspection manifest' 'payload manifest' 'SOURCE_SEMANTICS_AUTHENTICATED' 'SOURCE_EXPRESSIONS_AUTHENTICATED_NO_RUNTIME_FORMULA_EXECUTED' 'INSPECTION_ONLY' 'INSPECTION_ERROR' 'AUTHENTICATED_EVIDENCE_COMPLETE' 'SOURCE_ONLY_COMPLETE' 'SOURCE_ONLY_STATS_CONFIG_IDENTITY' 'NO_CHECKPOINT' 'NO_BPE' 'NO_UPLOAD' 'exit 2' 'CARGO_BUILD_JOBS=1' 'materialized_files' 'findmnt' 'README.md' 'cc-by-4.0' 'espnet/yodas_owsmv4' 'RepoFile' 'RepoFolder' 'classify_entry' 'OWSM_HF_TREE_SELF_TEST' 'approval-evidence' 'BLOCKED_APPROVAL' 'OWSM_ALLOW_OFFICIAL_REFERENCE' 'SOURCE_CONTRACT_AUTHENTICATED' '--config' '--stats' 'OFFICIAL_ESPnet_DEFAULT_FRONTEND_THEN_GLOBAL_MVN' 'normalize.mean' 'normalize.std' 'import escape' 'pre-existing espnet module contamination' 'dependency_audit.py' 'dependency-audit.json' '--validate-evidence' 'known factual blocker' 'primary license bytes unavailable: torch-complex==0.4.4' 'write_atomic_no_replace' 'ensure_dir' 'O_DIRECTORY' 'O_NOFOLLOW'; do if ! grep -Fq -- "$token" "$path" && ! grep -Fq -- "$token" "$INSPECTOR" && ! grep -Fq -- "$token" "$PREPARER" && ! grep -Fq -- "$token" "$FRONTEND_REFERENCE" && ! grep -Fq -- "$token" "$DEPENDENCY_AUDIT"; then echo "missing contract $token" >&2; fail=1; fi; done; if grep -En 'git[[:space:]]+push|upload\.sh|publish-one\.sh|--push|--upload' "$path" | grep -v 'grep -En' >/dev/null; then fail=1; fi; local source_only_block source_only_line approval_line; source_only_block="$(awk '/^source_only\(\)/{inside=1} /^self_test\(\)/{inside=0} inside' "$path")"; if printf '%s\n' "$source_only_block" | grep -Eq "$CHECKPOINT_RELATIVE|bpe_unigram50000|\.pth"; then echo 'source-only mode references checkpoint/BPE' >&2; fail=1; fi; source_only_line="$(grep -n '^if .*--source-only' "$path" | cut -d: -f1)"; approval_line="$(grep -n '^approval_path=' "$path" | cut -d: -f1)"; if [[ -z "$source_only_line" || -z "$approval_line" || "$source_only_line" -ge "$approval_line" ]]; then echo 'source-only branch is not before approval parsing' >&2; fail=1; fi; if ( source_only --expected-head 0000000000000000000000000000000000000000 ) 2>/dev/null; then echo 'source-only wrong/dirty HEAD accepted' >&2; fail=1; fi; UV_NO_CACHE=1 uv run --no-sync --frozen --offline --project "$ROOT/tools/parity" --python 3.12 python "$INSPECTOR" --self-test || fail=1; UV_NO_CACHE=1 uv run --no-cache --no-project --offline --python 3.12 python "$PREPARER" --self-test || fail=1; UV_NO_CACHE=1 uv run --no-cache --no-project --offline --python 3.12 python "$FRONTEND_REFERENCE" --self-test || fail=1; UV_NO_CACHE=1 uv run --no-cache --no-project --offline --python 3.12 python "$DEPENDENCY_AUDIT" --self-test || fail=1; if ! OWSM_HF_TREE_SELF_TEST=1 UV_NO_CACHE=1 uv run --no-cache --no-project --offline --python 3.12 python - <<'PY'
try:
...
[[ "$(uname -s)" == Linux && "$(uname -m)" == x86_64 ]] || die 'VAST requires Linux x86_64'; [[ "${VOKRA_PUBLISH_ON_VAST:-0}" == 1 ]] || die 'VOKRA_PUBLISH_ON_VAST=1 is absent'; [[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=all)" ]] || die 'checkout must be clean'
 mem_kib="$(awk '$1=="MemTotal:"{print $2;exit}' /proc/meminfo)"; [[ "$mem_kib" =~ ^[0-9]+$ && $mem_kib -ge $MIN_MEM_KIB ]] || die '128 GiB memory guard failed'; [[ "$(findmnt -T /dev/shm -n -o FSTYPE 2>/dev/null)" == tmpfs ]] || die '/dev/shm must be tmpfs'; free_kib="$(df -Pk /dev/shm | awk 'NR==2{print $4}')"; [[ "$free_kib" =~ ^[0-9]+$ && $free_kib -ge $MIN_DISK_KIB ]] || die 'tmpfs disk guard failed'; for command in cargo git uv awk find df findmnt readelf; do command -v "$command" >/dev/null || die "missing tool: $command"; done

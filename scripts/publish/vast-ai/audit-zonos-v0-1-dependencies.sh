...
self_test() {
  ...
  local bytecode_token="PYTHONDONTWRITEBYTECODE="'1'
  [[ "$(grep -Fc "$bytecode_token" "$0")" == 2 ]] || failed=1
  for forbidden in \
    'git '"clone" \
    'hugging'"face" \
    'H'"F_" \
...
set +e
PYTHONDONTWRITEBYTECODE=1 UV_PROJECT_ENVIRONMENT="$environment" UV_CACHE_DIR="${ZONOS_UV_CACHE_DIR:-/tmp/vokra-zonos-uv-cache}" \

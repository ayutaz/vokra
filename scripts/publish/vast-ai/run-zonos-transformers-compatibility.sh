...
self_test() {
  ...
  for needle in "$SOURCE_REPOSITORY" "$SOURCE_REVISION" 'GIT_LFS_SKIP_SMUDGE=1' 'transformers_compatibility.py' 'DacModel' 'DacEncoderOutput' 'DacDecoderOutput' 'DacConfig' 'n_codebooks' 'audio_codes' 'audio_values' 'get_type_hints' 'PASS_COMPATIBLE' 'NO_UPLOAD' '--validate-evidence' 'HF_TOKEN' 'HUGGING_FACE_HUB_TOKEN' 'HUGGINGFACE_API_TOKEN' 'checkpoint_access' 'constructor_calls' 'PYTHONDONTWRITEBYTECODE' 'torch.jit.script' 'jit_script_guard'; do

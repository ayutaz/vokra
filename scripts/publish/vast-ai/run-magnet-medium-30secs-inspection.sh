#!/usr/bin/env bash
# VAST-only, no-upload MAGNeT Medium inspection preflight.
...
die(){ echo "magnet-medium-30secs: BLOCKED: $*" >&2; exit 2; }
log(){ echo "magnet-medium-30secs: $*" >&2; }
...
validate_work(){ local work="$1" approval="$2" canonical_work canonical_root approval_dir; [[ ! -e "$work" && ! -L "$work" ]] || die 'work-dir must be absent/nonexistent'; canonical_work="$(canonical_candidate "$work")"; canonical_root="$(canonical_candidate "$ROOT")"; approval_dir="$(cd -P "$(dirname "$approval")" && pwd -P)"; paths_overlap "$canonical_work" "$canonical_root" && die 'work-dir overlaps checkout'; paths_overlap "$canonical_work" "$approval_dir" && die 'work-dir overlaps approval'; paths_overlap "$canonical_work" "$(canonical_candidate "$PROJECT")" && die 'work-dir overlaps project'; return 0; }
...
print('authenticated model README/compression/state identities')
PY
}
...
print('authenticated AudioCraft source roles')
PY
}
...
self_test(){
 [[ $# == 1 ]] || die '--self-test accepts no arguments'; grep -Fq "$HF_REVISION" "$0" || die 'fixed model revision missing'
 grep -Eq 'python .*GATE.*--project' "$0" || die 'stdlib gate invocation missing'; grep -Fq -- '--no-project' "$0" || die 'gate must use no-project'; grep -Fq 'snapshot_download' "$0" || die 'authenticated collection stage missing'; grep -Fq 'git fetch --depth 1' "$0" || die 'authenticated source fetch missing'
 if "$0" --approval-evidence >/dev/null 2>&1 || "$0" --approval-evidence -bad >/dev/null 2>&1 || "$0" --approval-evidence one --approval-evidence two >/dev/null 2>&1 || "$0" --self-test x >/dev/null 2>&1; then die 'invalid option accepted'; fi
 UV_NO_CACHE=1 uv run --no-cache --no-project --offline --python 3.12 python "$GATE" --self-test
 self_test_work_paths; tmp="$(mktemp -d)"; trap 'rm -rf "$tmp"' EXIT; printf '{}' >"$tmp/evidence.json"; if "$0" --approval-evidence "$tmp/evidence.json" --work-dir "$tmp/work" >/dev/null 2>&1; then die 'pending production gate unexpectedly passed'; fi; [[ ! -e "$tmp/work" && ! -L "$tmp/work" ]] || die 'blocked gate created work-dir'
 echo 'run-magnet-medium-30secs-inspection.sh self-test: PASS'
}
...
if [[ "${1:-}" == --self-test ]]; then check_default_work; self_test "$@"; exit 0; fi

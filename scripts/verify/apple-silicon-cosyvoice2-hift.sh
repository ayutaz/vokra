...
reject_checked_in(){
  local candidate="$1" checked_in="$ROOT/tools/parity/cosyvoice2_hift_reference/license_gate_manifest.json"
...
def pairs(items):
    ...
    for key, value in items:
        ...
        result[key] = value
    return result
document = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"), object_pairs_hook=pairs)
...
validate_evidence_json(){
  local output="$1" gguf_sha="$2" reference_sha="$3" license_sha="$4"
...
if document["scope"] != {"component":"standalone_cosyvoice2_hift","f0":"CPU/reference only; Metal F0 not separately exposed","pcm":"CPU/reference, Metal/reference, Metal/CPU","full_cosvoice2_e2e":"NOT_CLAIMED"}: raise SystemExit("evidence scope mismatch")
for key in ("f0_cpu_reference_max_abs","pcm_cpu_reference_max_abs","pcm_metal_reference_max_abs","pcm_metal_cpu_max_abs"):
    value = document[key]
    if not isinstance(value, (int,float)) or not math.isfinite(value) or value < 0 or value > 0.01: raise SystemExit(f"invalid metric: {key}")
PY
}
self_test(){
  local fail=0 token duplicate_output temp
  ...
  if (disjoint /tmp /tmp/subtree) 2>/dev/null; then fail=1; fi
...
PY
  validate_preflight_json "$temp/preflight.json" || fail=1
  ...
  if validate_preflight_json "$temp/duplicate.json" >/dev/null 2>&1; then fail=1; fi
  ...
  printf '%s\n' '{' >"$temp/malformed.json"
  if validate_preflight_json "$temp/malformed.json" >/dev/null 2>&1; then fail=1; fi
...
parse_args(){
  GGUF=""; GGUF_SHA=""; REFERENCE=""; REFERENCE_SHA=""; LICENSE=""; EVIDENCE=""; local seen="" key value
  while [[ $# -gt 0 ]]; do
...
validate_preflight_json "$TMP/license.log" || die 'license preflight JSON was not an exact PASS object'
...
[[ "$(grep -Ec 'test result: ok\. 1 passed; 0 failed' "$TMP/cargo.log")" == 1 ]] || die 'Cargo did not report one passing result summary'

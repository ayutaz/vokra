...
require_tooling() {
  ...
  grep -Fq 'CANARY_1B_V2_METAL_VS_OFFICIAL PASS' <<<"$test_body" \
    || die "Canary-v2 test lacks its Metal-vs-official sentinel"
...
run_case() {
  ...
  [[ "$(grep -Fxc 'CANARY_1B_V2_CPU_VS_OFFICIAL PASS' "$log_path" || true)" == 1 ]] \
    || die "Canary-v2 ${language_pair} CPU-vs-official sentinel is absent"
  [[ "$(grep -Fxc 'CANARY_1B_V2_METAL_VS_OFFICIAL PASS' "$log_path" || true)" == 1 ]] \
    || die "Canary-v2 ${language_pair} Metal-vs-official sentinel is absent"
  [[ "$(grep -Fxc 'CANARY_1B_V2_METAL_VS_CPU PASS' "$log_path" || true)" == 1 ]] \
...
main() {
  ...
  [[ "$(grep -Fxc 'CANARY_1B_V2_CPU_VS_OFFICIAL PASS' \
    "$evidence_dir/parity-en-en.log" || true)" == 1 ]] \
    || die "Canary-v2 ASR CPU/official marker is absent"
  [[ "$(grep -Fxc 'CANARY_1B_V2_METAL_VS_OFFICIAL PASS' \
    "$evidence_dir/parity-en-en.log" || true)" == 1 ]] \
    || die "Canary-v2 ASR Metal/official marker is absent"
  ...
  [[ "$(grep -Fxc 'CANARY_1B_V2_METAL_VS_OFFICIAL PASS' \
    "$evidence_dir/parity-en-de.log" || true)" == 1 ]] \
    || die "Canary-v2 AST Metal/official marker is absent"
  [[ "$(grep -Fxc 'CANARY_1B_V2_METAL_VS_CPU PASS' \
    "$evidence_dir/parity-en-de.log" || true)" == 1 ]] \

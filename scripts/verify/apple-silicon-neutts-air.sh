...
require_reference() {
  ...
  require_file "reference manifest" "$directory/manifest.txt"
...
main() {
  local gguf='' companion='' reference='' evidence_dir='' self_test=0
  while (( $# > 0 )); do
    case "$1" in
      --gguf)
        [[ $# -ge 2 ]] || { usage; return 2; }
  ...
  if (( self_test == 1 )); then
    [[ -z "$gguf$companion$reference$evidence_dir" ]] \
      || die "--self-test accepts no other arguments"
    run_self_test
    return
  fi
  [[ -n "$gguf" && -n "$companion" && -n "$reference" && -n "$evidence_dir" ]] \
    || { usage; die "--gguf, --companion, --reference and --evidence-dir are required"; }

  require_remote_apple_host
  require_tooling
  require_identity "NeuTTS Air GGUF" "$gguf" "$PUBLIC_BYTES" "$PUBLIC_SHA256"
  ...
  grep -F 'NEUTTS_AIR_COMPOSITION Cpu' "$evidence_dir/parity-cpu.log" >/dev/null \
    || die "CPU composition marker is absent"
  ...
  grep -F 'NEUTTS_AIR_PARITY Metal_vs_official logits_atol=0.01 greedy_ids=exact PASS' \
    "$evidence_dir/parity-metal.log" >/dev/null \
    || die "Metal PASS marker is absent"
  grep -F 'NEUTTS_AIR_COMPOSITION Metal' "$evidence_dir/parity-metal.log" >/dev/null \
    || die "Metal composition marker is absent"

#!/usr/bin/env bash
# Real-weight NVIDIA Canary-1B-Flash CPU/reference/Metal parity on a
...
# mutates model artifacts.  The exact real-weight test is deliberately
# fail-closed if the checkout has not yet gained its explicit Metal leg.
...
and English-to-German AST cases on a disposable Darwin/arm64 host.  The test
must contain both the independent official-NeMo comparison and its explicit
...
require_cpu_evidence() {
  ...
  for marker in \
    "format=canary-1b-cpu-evidence-v1" "variant=canary-1b-flash" \
...
require_reference() {
  ...
  for suffix in en-en en-de; do
    for name in json pcm.f32 tokens.txt text.txt; do
      require_file "Canary reference ${suffix}.${name}" \
...
require_tooling() {
  ...
  [[ -f "$PARITY_SOURCE" ]] \
    || die "Canary-1B-Flash parity source is missing: $PARITY_SOURCE"
  grep -Fq "fn $TEST_NAME" "$PARITY_SOURCE" \
    || die "Canary real-weight test is missing: $TEST_NAME"
  grep -Fq 'official NeMo token fixture' "$PARITY_SOURCE" \
    || die "Canary test lacks the independent official-NeMo token input"
  grep -Fq 'Canary1bFlashAsr::from_gguf_with_backend(&gguf, BackendKind::Cpu)' \
    "$PARITY_SOURCE" \
    || die "Canary test lacks its CPU real-weight bind"
  ...
  [[ "$test_line" =~ ^[0-9]+$ ]] || die "could not locate Canary real-weight test body"
  test_body="$(sed -n "${test_line},$((test_line + 180))p" "$PARITY_SOURCE")"
  grep -Fq 'Canary1bFlashAsr::from_gguf_with_backend' <<<"$test_body" \
    || die "Canary test has no explicit real-weight Metal bind; refusing CPU-only PASS"
  grep -Fq 'BackendKind::Metal' <<<"$test_body" \
    || die "Canary real-weight test lacks a Metal-vs-CPU leg"
  grep -Fq 'assert_eq!(metal_actual, actual, "Flash Metal IDs equal CPU");' \
    <<<"$test_body" \
...
run_self_test() (
  ...
  printf '%s\n' "test $TEST_TARGET ... ok" 'test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out; finished in 0.01s' 'CANARY_1B_FLASH_CPU_VS_OFFICIAL PASS' > "$cpu_log"
  digest="$(sha256_file "$cpu_log")"
  require_cpu_log "$cpu_log" "$digest" "$TEST_TARGET" 'CANARY_1B_FLASH_CPU_VS_OFFICIAL PASS'
  printf '%s\n' 'test result: ok. 0 passed; 0 failed; 1 ignored; 0 measured; 0 filtered out; finished in 0.01s' > "$cpu_empty"
  if require_cpu_log "$cpu_empty" "$(sha256_file "$cpu_empty")" "$TEST_TARGET" 'CANARY_1B_FLASH_CPU_VS_OFFICIAL PASS' >/dev/null 2>&1; then die "zero-test CPU log accepted"; fi
  printf '%s\n' \
    'format=canary-1b-portable-transfer-v1' "expected_head=$expected_head" \
  ...
  for required in \
    ...
    'network=NOT_PERFORMED' 'conversion=NOT_PERFORMED' 'format=canary-1b-portable-transfer-v1' \
    'apple-transfer-manifest.txt' '<APPLE_CPU_ASR_LOG>' '<APPLE_CPU_AST_LOG>' '<APPLE_TRANSFER_MANIFEST>'; do
...
main() {
  ...
  require_cpu_log "$cpu_asr_log" "$cpu_asr_log_sha256" "$TEST_TARGET" 'CANARY_1B_FLASH_CPU_VS_OFFICIAL PASS'
  require_cpu_log "$cpu_ast_log" "$cpu_ast_log_sha256" "$TEST_TARGET" 'CANARY_1B_FLASH_CPU_VS_OFFICIAL PASS'
  ...
  log "running exact real-weight Canary ASR CPU/official/Metal comparison"

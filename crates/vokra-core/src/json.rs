...
//!
//! Just enough to read a [`safetensors`](crate::safetensors) header (objects,
//! arrays, strings, integers, floats, booleans, null). Numbers are parsed with
//! the locale-independent [`str::parse`] — never `strtod` (NFR-RL-01), which
//! misparses or crashes under European comma-decimal locales.
//!
//! # Why this is in `vokra-core`
//!
//! Promoted here from `vokra-convert` in M1-02 so the *runtime* safetensors
//! direct-load path (FR-LD-04 / IF-06) and the offline conversion tool can
//! share one hand-written, std-only parser. It carries no external dependency
//! and never touches ONNX/protobuf, so it does not widen the runtime surface
//! (NFR-DS-02).

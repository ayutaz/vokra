...
//! The bound is `2e-5` max absolute error. RNNoise's pitch tracker (upstream
//! `src/pitch.c`) and filterbank (upstream `src/denoise.c`) remain waveform
//! determinism choke-points. The legacy pitch check is a *lag-band*
...
//!
//! Full waveform parity remains explicit until the legacy 22-band/42-feature
//! `vokra_ops::rnnoise` DSP is replaced by the v0.2 32-band/65-feature path.

...
// ---------------------------------------------------------------------------
// speaker-diarization-3.1 pipeline GGUF contract
// ---------------------------------------------------------------------------

/// Architecture tag of the public weightless pipeline GGUF.
///
/// This is named `MODEL_TAG` rather than `ARCH` until the complete CLI
/// execution route lands: the bound-arch registry is reserved for model
/// handles with a callable forward, while this commit deliberately adds only
/// the orchestration-config binder.
...
/// Loading therefore rejects both missing metadata and any tensor payload;
/// accepting a generic zero-tensor GGUF would make an unrelated config look
/// runnable (FR-EX-08).
#[derive(Debug, Clone, PartialEq)]
pub struct PyannoteSpeakerDiarization31Config {
    /// Upstream segmentation model identifier.
    pub segmentation_model: String,
    /// Upstream segmentation batch size (traceability; Vokra may execute serially).
    ...
    /// Upstream embedding batch size (traceability; Vokra may execute serially).
    ...
    pub fn from_gguf(file: &GgufFile) -> Result<Self> {
        ...
        if license.class != LicenseClass::Permissive {
            return Err(VokraError::ModelLoad(format!(
                "pyannote-speaker-diarization-3.1: weight license resolves to {}, expected permissive MIT",
                license.class.as_str()
            )));
        }
        if !file.tensors().is_empty() {
            return Err(VokraError::ModelLoad(format!(
                "pyannote-speaker-diarization-3.1: pipeline GGUF must be weightless, found {} tensor(s)",
                file.tensors().len()
            )));
        }
        ...
        for (key, actual, expected) in [
            ...
            if actual != expected {
                return Err(VokraError::ModelLoad(format!(
                    "pyannote-speaker-diarization-3.1: `{key}` is {actual}, expected {expected}"
                )));
            }
        }
        if segmentation_min_duration_off.to_bits() != 0.0f32.to_bits() {
            return Err(VokraError::ModelLoad(format!(
                "pyannote-speaker-diarization-3.1: `{KEY_SEGMENTATION_MIN_DURATION_OFF}` is {segmentation_min_duration_off}, expected 0"
            )));
        }
        if !embedding_exclude_overlap {
            return Err(VokraError::ModelLoad(format!(
                "pyannote-speaker-diarization-3.1: `{KEY_EMBEDDING_EXCLUDE_OVERLAP}` is false, expected true"
            )));
        }
        if clustering_threshold.to_bits() != EXPECTED_CLUSTERING_THRESHOLD.to_bits() {
            return Err(VokraError::ModelLoad(format!(
                "pyannote-speaker-diarization-3.1: `{KEY_CLUSTERING_THRESHOLD}` is {clustering_threshold}, expected {EXPECTED_CLUSTERING_THRESHOLD}"
            )));
        }
    ...
    let actual = file
        ...
        .ok_or_else(|| {
            VokraError::ModelLoad(format!(
                "pyannote-speaker-diarization-3.1: missing string `{key}`"
            ))
        })?;
    if actual != expected {
        return Err(VokraError::ModelLoad(format!(
...
fn required_pipeline_u32(file: &GgufFile, key: &str) -> Result<u32> {
    match file.get(key) {
        ...
        _ => Err(VokraError::ModelLoad(format!(
            "pyannote-speaker-diarization-3.1: missing u32 `{key}`"
        ))),
    }
}
...
fn required_pipeline_f32(file: &GgufFile, key: &str) -> Result<f32> {
    match file.get(key) {
        ...
        _ => Err(VokraError::ModelLoad(format!(
            "pyannote-speaker-diarization-3.1: missing finite f32 `{key}`"
        ))),
    }
}
...
fn required_pipeline_bool(file: &GgufFile, key: &str) -> Result<bool> {
    match file.get(key) {
        ...
        _ => Err(VokraError::ModelLoad(format!(
            "pyannote-speaker-diarization-3.1: missing bool `{key}`"
        ))),
    }
}

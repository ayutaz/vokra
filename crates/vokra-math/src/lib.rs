//! Shared scalar `f32` transcendental primitives for Vokra —
//! `exp` / `tanh` / `sqrt` / `sin` / `cos` / `log` / `log1p` in pure `core`
//! arithmetic (WP-07 extraction).
//!
//! # Provenance
//!
//! The implementations were born at
//! `crates/vokra-backend-cpu/src/kernels/scalar_transcendental.rs` — Wave 1
//! (`exp`/`tanh`/`sqrt`, M5-03-T06) added the first three for the no_std
//! Silero VAD subset; **WP-06** (2026-08-09, commit `2f1696c`) added the
//! `sin` / `cos` / `log` / `log1p` family for the Style-Bert-VITS2 v2 hot
//! path. **WP-07** hoists them into this crate — a first-party `vokra-*`
//! crate whose only dependency is `core` — so sibling non-backend crates
//! (`vokra-ops::hifigan`, `vokra-bert::deberta_v2`, and future SBV2
//! kernels) can call them WITHOUT depending on `vokra-backend-cpu`. That
//! edge would flip the crate graph inside-out (backend on top of ops, not
//! the other way round) and pull the full CPU kernel tier — AVX-512,
//! dotprod / i8mm, K-quants, the thread pool — into every op crate.
//! `vokra-backend-cpu` now depends on this crate and re-exports the
//! primitives from its `scalar_transcendental` module for backward
//! compatibility.
//!
//! # Why bespoke primitives at all
//!
//! The Silero VAD forward calls `exp` (sigmoid), `tanh` (LSTM cell) and
//! `sqrt` (pseudo-STFT magnitude); the SBV2 SDP flow / ElementwiseAffine
//! path calls `log`, `log1p`, `sin`, `cos`. Those `f32` transcendentals
//! live in **`std`**, not `core`: on the Cortex-M55 (`thumbv8m-none`)
//! Tier-3 target (NFR-PT-03) `f32::exp` / `tanh` / `sqrt` do not resolve
//! (measured: `E0599 no method named 'exp'/'tanh'/'sqrt' found for type
//! 'f32'`). The obvious fix — the `libm` crate — is **forbidden**: it is
//! a non-`vokra-*` crates.io dependency and would break the zero-dependency
//! invariant (NFR-DS-02; `scripts/check-zero-deps.sh` fails on any
//! non-vokra entry in `Cargo.lock`, and `deny.toml` bans `libm`).
//!
//! So the transcendentals are supplied here in pure `core` arithmetic,
//! **not copied** from `libm` / Cephes / SLEEF / Pommier (license hygiene;
//! Vokra stays zero-dependency and Apache-2.0). Every function is
//! deterministic across targets (plain `f32` ops, no platform rounding-mode
//! or FMA dependence), which is what lets a later wave make the std and
//! no_std Silero forwards bit-identical **by construction** (M5-03 T08/T11)
//! and what lets the SBV2 SIMD wrapper layer decide FMA on its own terms
//! (WP-06 module doc).
//!
//! # Bit-exactness discipline (workspace red line)
//!
//! - **No `f32::mul_add`** here — plain `*` + `+` keeps every result
//!   deterministic across FMA-capable and FMA-less targets. Cross-plat
//!   bit-exactness against the SIMD wrappers is handled at the SIMD side
//!   by inserting `mul_add` on both AVX2 and NEON micro-kernels (or their
//!   soft-emulated fallback), NOT here.
//! - **No `unsafe`.**
//! - **Coefficients are exact rational reciprocals** (`1/n!` for exp / sin
//!   / cos, `1/(2k+1)` for the atanh expansion `log` rides on top of).
//!   Nothing is scavenged from third-party approximation tables, so this
//!   crate needs no third-party attribution.
//!
//! # `sqrt` route (undecided — ADR M5-03 §sqrt)
//!
//! [`sqrt`] is a Newton–Raphson refinement: portable, `core`-only, no
//! `unsafe`, deterministic across targets. It is **not** bit-identical to
//! a hardware `vsqrt` (IEEE correctly-rounded) result — it deviates by a
//! bounded few ULP. The Silero pseudo-STFT declares itself an NFR-QL-05
//! red line (upstream-faithful `magnitude = sqrt(re²+im²)`); whether a
//! later wave keeps this Newton `sqrt` or switches to an
//! `asm!("vsqrt.f32 …")` (HW, IEEE-exact, but `unsafe` + FP-armv8-only) is
//! an owner decision recorded in the ADR — it is **undecided** and this
//! Newton path is the working default, not a ratified choice.

// The crate is `#![no_std]` whenever the default `std` feature is off, so
// `#![no_std]` targets (Cortex-M55 Tier 3, NFR-PT-03) can pull it in
// alongside `vokra-vad-micro`. Every function uses only `core` arithmetic
// (verified: the implementations use `f32::clamp` / `abs` / `is_nan` /
// `is_infinite` / `to_bits` / `from_bits`, all of which are `core`; plus
// constants under `core::f32::consts::*`). With the default feature set
// this attribute is inert — the crate compiles as a normal std library.
// This mirrors `vokra-vad-micro`'s NFR-PT-01 cross-build non-interference.
...
mod tests {
    ...
    pub(super) const LOG_REL_TOL: f32 = 5.0e-7;

...
//!
//! Meta AudioCraft's Joint Audio-Symbolic COnditioning music
//! generation model (Tal et al. 2024 "Joint Audio and Symbolic
...
//!
//! # BF16 pass-through (mirror of musicgen_medium / audiogen_medium /
...
/// `vokra.model.arch` value for JASCO 400M Chords+Drums GGUFs.
/// Distinct from every sibling music-generation family — silently
/// sharing an arch tag with `musicgen` (Meta AudioCraft AR over
/// EnCodec), `audioldm2` (latent diffusion), `stable_audio_open_small`
/// (DiT + VAE), `ace_step`, `bs_roformer` (music-source separation),
/// or `yue_bundle` would mis-route the runtime dispatch. JASCO's
/// joint text + chord + drum conditioning stack is a distinct
/// topology.
...
/// Shared with the sibling MusicGen family (`music` — 2026-07-30
/// scope expansion `[[project-scope-expansion-2026-07-30]]`).
...
/// Converts a JASCO 400M Chords+Drums safetensors checkpoint at
...
pub fn convert_jasco_400m_chords_drums_file(
    input: &Path,
    output: &Path,
    license: Option<&str>,
) -> Result<Jasco400mChordsDrumsReport, ConvertError> {
    ...
    let mut report = Jasco400mChordsDrumsReport::default();
    for t in st.tensors() {
        report.read += 1;
        match t.dtype {
...
mod tests {
    ...
    fn scratch_path(tag: &str, ext: &str) -> PathBuf {
        ...
        p.push(format!(
            "vokra-jasco-400m-chords-drums-{tag}-{}-{}.{ext}",
            std::process::id(),
    ...
    fn bf16_tensor_passes_through_verbatim() {
        ...
        let report = convert_jasco_400m_chords_drums_file(&input_path, &output_path, None)
            .expect("convert BF16");
    ...
    fn license_override_replaces_default() {
        ...
        let report =
            convert_jasco_400m_chords_drums_file(&input_path, &output_path, Some("apache-2.0"))

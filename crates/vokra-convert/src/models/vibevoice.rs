...
pub(crate) struct VibeVoiceReport {
    ...
    /// Non-F32 / F16 tensors skipped (defensive counter — the
    /// safetensors reader rejects unknown dtypes at parse time, but
    /// BF16 does reach this arm since M4-06).
    ...
    if report.written == 0 {
        report.notes.push(
            "no float tensors passed through — this GGUF is metadata-only and \
             ...
             (config.json `torch_dtype: bfloat16`); pre-widen offline to F32 or \
             wait for the streaming BF16 pass-through path (T29-equivalent — \
...
mod tests {
    ...
    /// serving format for VibeVoice-1.5B) must reach the
    ...
    fn bf16_tensor_is_counted_as_skipped_non_float() {
        ...
        assert_eq!(
            report.written, 0,
            "BF16 must not currently pass through — VibeVoice converter is F32/F16 only"
        );
        assert_eq!(
            report.skipped_non_float, 1,
        ...
        assert!(
            file.tensor_info("model.embed_tokens.weight").is_none(),

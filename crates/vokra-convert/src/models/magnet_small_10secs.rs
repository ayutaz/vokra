...
//! Non-Autoregressive Transformer — 500M parameter variant that
//! generates 10-second music clips via **parallel masked-LM
//! decoding** (Ziv et al. 2024 arXiv:2401.04577 "Masked Audio
//! Generation using a Single Non-Autoregressive Transformer"),
//! distributed on HuggingFace at
//! `huggingface.co/facebook/magnet-small-10secs`. Weight licence is
//! **CC-BY-NC-4.0** (research-only, T4 tier — MusicGen family /
//! X-Codec-2 precedent), so publish requires
...
//! AR baselines). Silently sharing the `musicgen` arch tag with
//! sibling Meta AudioCraft AR-over-EnCodec releases
//! (`musicgen-medium` / `musicgen-large` / `musicgen-small`) would
//! mis-route the runtime dispatch: MAGNeT's decoding schedule
//! (`magnet_masked_decode`) and sampling primitives (span masking
//! + confidence estimation) are a different code path from
//! MusicGen's autoregressive token-by-token generation. This
//! converter stamps a distinct `vokra.model.arch = "magnet_small_10secs"`
//! so a future runtime binder cannot accidentally silently share the
//! MusicGen loader.
//!
//! # BF16 pass-through (mirror of jasco_400m_chords_drums /
//! # musicgen_medium / audiogen_medium / xcodec2 /
...
//! (sibling to musicgen_medium / audiogen_medium / audioldm2 /
//! jasco_400m_chords_drums — the runtime binder `vokra_models::magnet`
//! relies on the upstream key set without a rename layer). That binder
//! has landed as a scaffold carrying `ARCH_SMALL = "magnet_small_10secs"`
//! matching this converter's stamp: `MagnetEngine::from_gguf` really
//! validates the arch tag, deserialises `vokra.magnet.*` and catalogues
//! these tensor names. Runtime `magnet_masked_decode` +
//! `span_masking_scheduler` ops are the FR-OP-85 anchor for a follow-up
//! wave (owner ADR judgement); what stays deferred is
//! `MagnetEngine::forward`, which is loud-partial per the RMVPE /
//! Charsiu / MOSS-Audio-Tokenizer / MioCodec precedent — not the binder.
//!
//! # No ONNX (permanent)
//!
...
/// `vokra.model.arch` value for MAGNeT Small 10secs GGUFs. Distinct
/// from every sibling music-generation family — silently sharing an
/// arch tag with `musicgen_medium` / `musicgen_large` /
/// `musicgen_small` (Meta AudioCraft **AR** over EnCodec),
/// `audiogen_medium` (AR SFX / environmental sound sibling of
/// MusicGen), `audioldm2` (latent diffusion), `stable_audio_open_small`
/// (DiT + VAE), `jasco_400m_chords_drums` (joint audio-symbolic
/// conditioning), `ace_step`, `yue_bundle`, or `bs_roformer`
/// (music-source separation) would mis-route the runtime dispatch.
/// MAGNeT's non-autoregressive masked-LM parallel decoding stack is
/// a distinct topology.
pub const ARCH: &str = "magnet_small_10secs";

/// `vokra.model.name` value written for the canonical
/// `facebook/magnet-small-10secs` release.
pub const NAME: &str = "magnet_small_10secs";

/// `vokra.model.category` value written for every MAGNeT Small 10secs
/// GGUF. Shared with the sibling MusicGen / AudioGen / JASCO family
/// (`music` — 2026-07-30 scope expansion
...
/// `huggingface.co/facebook/magnet-small-10secs`.
pub const UPSTREAM_HF: &str = "facebook/magnet-small-10secs";

/// Default upstream weight licence (SPDX). Verified against the
/// upstream HF card — CC-BY-NC-4.0 (research-only, non-commercial,
/// T4 tier per MusicGen family / X-Codec-2 precedent).
...
/// Outcome of a MAGNeT Small 10secs conversion.
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub struct MagnetSmall10secsReport {
...
/// Converts a MAGNeT Small 10secs safetensors checkpoint at `input`
/// into a Vokra-native GGUF at `output`, returning a
/// [`MagnetSmall10secsReport`].
///
...
pub fn convert_magnet_small_10secs_file(
    input: &Path,
    output: &Path,
    license: Option<&str>,
) -> Result<MagnetSmall10secsReport, ConvertError> {
    ...
    vokra_core::stamp_provenance(
        ...
        Some(NAME),
        Some(
            "facebook/magnet-small-10secs (Meta AudioCraft Masked Audio Generation, \
             non-autoregressive masked-LM parallel decoding for 10-second music \
             generation, Ziv et al. 2024 arXiv:2401.04577, CC-BY-NC-4.0 — owner \
             §3.1 sign-off required, publish requires --allow-noncommercial per \
             MusicGen family T4 precedent)",
        ),
    );

    let mut report = MagnetSmall10secsReport::default();
    for t in st.tensors() {
        report.read += 1;
        match t.dtype {
...
mod tests {
    ...
    fn scratch_path(tag: &str, ext: &str) -> PathBuf {
        ...
        p.push(format!(
            "vokra-magnet-small-10secs-{tag}-{}-{}.{ext}",
            std::process::id(),
    ...
    fn bf16_tensor_passes_through_verbatim() {
        ...
        let report = convert_magnet_small_10secs_file(&input_path, &output_path, None)
            .expect("convert BF16");
    ...
    fn license_override_replaces_default() {
        ...
        let report =
            convert_magnet_small_10secs_file(&input_path, &output_path, Some("apache-2.0"))
    ...
    #[test]
    fn arch_and_name_are_stable_constants_distinct_from_musicgen_family() {
        // Guards the FR-EX-08 dispatch boundary: MAGNeT non-autoregressive
        // masked-LM must not silently share the `musicgen` arch tag with
        // sibling AR-over-EnCodec releases. A future rename of this
        // constant would break runtime dispatch downstream — this pin
        // makes such a change require an explicit ADR / test update.
        assert_eq!(ARCH, "magnet_small_10secs");
        assert_eq!(NAME, "magnet_small_10secs");
        assert_ne!(
            ARCH, "musicgen",
    ...
    fn upstream_and_category_pin_expected_values() {
        ...
        assert_eq!(UPSTREAM_HF, "facebook/magnet-small-10secs");

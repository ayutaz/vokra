...
pub fn convert_canary_1b_flash_file_with_tokenizer(
    ...
    if tokenizer_vocab.is_symlink() || !tokenizer_vocab.is_file() {
        return Err(ConvertError::Usage(format!(
            "canary-1b-flash tokenizer must be a regular non-symlink file: {}",
    ...
    if output.exists() || output.is_symlink() {
        return Err(ConvertError::Usage(format!(
            "canary-1b-flash output already exists or is symlinked: {}",
            output.display()
        )));
    }
    if input.is_symlink() || !input.is_file() {
        return Err(ConvertError::Usage(format!(
            "canary-1b-flash checkpoint must be a regular non-symlink file: {}",
            input.display()
        )));
    }
...
fn validate_io_paths(
    ...
    let parent = output.parent().ok_or_else(|| {
        ConvertError::Usage("canary-1b-flash output must have a parent directory".to_owned())
    })?;
    if !parent.is_dir() || parent.is_symlink() {
        return Err(ConvertError::Usage(format!(
            "canary-1b-flash output parent must be an existing regular non-symlink directory: {}",
            parent.display()
        )));
    }
    Ok(())
...
fn reject_unsafe_path(path: &Path, label: &str) -> Result<(), ConvertError> {
    ...
    {
        return Err(ConvertError::Usage(format!(
            "canary-1b-flash {label} must not contain lexical dot components"
        )));
    }
    ...
    loop {
        if current.is_symlink() && current != Path::new("/var") {
            return Err(ConvertError::Usage(format!(
                "canary-1b-flash {label} has symlink ancestry: {}",
                current.display()
            )));
        }
...
mod tests {
    ...
    fn output_writer_never_clobbers_claimed_final_path() {
        let root =
            std::env::temp_dir().join(format!("vokra-canary-flash-output-{}", std::process::id()));
    ...
    fn io_paths_reject_dot_components_and_symlink_ancestors() {
        let root =
            std::env::temp_dir().join(format!("vokra-canary-flash-paths-{}", std::process::id()));
    ...
    fn dedicated_path_authenticates_tokenizer_before_checkpoint_io() {
        let root = std::env::temp_dir().join(format!(
            "vokra-canary-flash-tokenizer-first-{}",
            std::process::id()
        ));
        ...
        let error =
            convert_canary_1b_flash_file_with_tokenizer(&checkpoint, &output, None, &tokenizer)

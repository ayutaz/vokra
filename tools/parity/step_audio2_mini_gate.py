...
"""Stdlib-only terminal gate for the unresolved Step-Audio 2 Mini composite."""
...
MODEL_REPOSITORY = "stepfun-ai/Step-Audio-2-mini"
MODEL_REVISION = "e36fdd5d71e0ea22f09dd94bbab9bfc544ca1e36"
...
BLOCKED_MARKER = "BLOCKED_UNRESOLVED_STEP_AUDIO2_MINI_COMPOSITE"
...
def _fixture(head: str) -> dict[str, Any]:
    data: dict[str, Any] = {
        "schema": "vokra-step-audio2-mini-approval-v1",
        "status": "BLOCKED",
        ...
        "source_role_status": "BLOCKED_UNRESOLVED",
        "dependency_status": "BLOCKED_UNREVIEWED",
        "license_status": "BLOCKED_COMPONENT_REVIEW",
        "dataset_status": "BLOCKED_UNAUTHENTICATED",
        "tokenizer_vocoder_status": "UNRESOLVED",
...
def require_blocked_gate(head: str, approval: str, supplied_sha: str, root: Path) -> None:
    ...
    raise RuntimeError(f"{BLOCKED_MARKER}: native S2S, dependencies, dataset, and composite provenance remain unresolved")
...
def self_test(source: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="step-audio2-gate-") as repo_tmp, tempfile.TemporaryDirectory(prefix="step-audio2-approval-") as approval_tmp:
        repo = Path(repo_tmp).resolve()
        subprocess.run(["git", "-C", str(repo), "init", "-q"], check=True, capture_output=True)
        subprocess.run(["git", "-C", str(repo), "config", "user.name", "Step-Audio2 self-test"], check=True)
        subprocess.run(["git", "-C", str(repo), "config", "user.email", "step-audio2-self-test@example.invalid"], check=True)
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("step_audio2_mini_gate self-test: OK")
        return 0

#!/usr/bin/env python3
"""Bridge FireRedTeam/FireRedASR-LLM-L sharded safetensors → flat safetensors.

Offline sidecar tool (FR-LD-05: no Python / PyTorch ever enters the
runtime). The upstream `FireRedTeam/FireRedASR-LLM-L` release ships
**sharded safetensors** (`model-00001-of-000NN.safetensors` +
`model.safetensors.index.json`, ~16.6 GB total in BF16 for the 8.3B
backbone = Conformer encoder + audio-to-text adapter + Qwen2 LM
decoder). The Rust converter
(``crates/vokra-convert/src/models/firered_asr_llm_l.rs``) consumes a
**single** safetensors file so this script merges the shards, dedupes
tied tensors, strips non-float training-scaffold state, and writes
one flat output. Mirror of the sibling
...
* Dedupes shared-storage tensors via a `data_ptr → canonical name`
  ...
  FireRedASR-LLM-L topology since its LM decoder is Qwen2-family).
...
The 8.3B BF16 weights are ~16.6 GB on disk. This script loads shards
sequentially and merges into a single in-memory state dict — peak
resident memory is roughly the whole model (~16.6 GB) because
`safetensors.torch.save_file` needs the full dict for its serialise
call. This is the wave-B posture (magpietts_v2602 /
firered_asr_aed_l / higgs_audio_v3_tts_4b). On the CC laptop
(M1 iMac 16 GB) this exceeds the safe zone — per memory
`[[feedback-large-models-on-vast-ai]]` the actual conversion runs
on vast.ai per `docs/handoff/vast-ai-large-model-publish.md` (rent
a box with ≥ 32 GB RAM to comfortably hold the whole state dict +
...
::
    ...
    cd tools/parity/firered_asr_llm_l
    uv sync
...
::
    ...
    vokra-cli convert --model firered-asr-llm-l \\
...
def dedupe_shared_storage(
    ...
    In the FireRedASR-LLM-L family a plausible tie is the LM decoder's
    text embedding ↔ lm_head (Qwen2 `tie_word_embeddings=true`
    posture). The audit trail preserves that fact even after we
    clone-and-detach the alias so the writer stops rejecting.
    """
    seen: dict[int, str] = {}
...
def main() -> int:
    ...
    src.add_argument(
        ...
        "(upstream `FireRedTeam/FireRedASR-LLM-L` layout after `hf download`)",
    )
    src.add_argument(
        "--shards",
        nargs="+",

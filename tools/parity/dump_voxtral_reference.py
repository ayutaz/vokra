...
BF16 shards by default, or another explicitly named real Voxtral checkpoint
(BF16 → fp32 widening is exact, so weight representation is
...
def write_manifest(out_dir: Path, manifest: dict[str, object]) -> None:
...
def rss_gib() -> float:
    ...
    # macOS reports bytes; Linux (the VAST execution environment) reports KiB.
    return peak / (2**30 if sys.platform == "darwin" else 2**20)


def build_prompt(ckpt: Path, audio_path: Path, request_model: str):
    ...
    req = TranscriptionRequest(
        model=request_model, audio=RawAudio.from_audio(audio), language="en"
    )
...
def run_tower(ckpt: Path, pcm: np.ndarray, full_taps: Path | None):
    ...
    if not (fe.feature_size == 128 and fe.hop_length == 160 and fe.n_fft == 400):
        sys.exit(
            ...
            f"hop={fe.hop_length}, n_fft={fe.n_fft}) — not the shipping Voxtral 2507 family"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "LOCAL HF snapshot of a real Voxtral release (config.json "
            "+ model-*.safetensors + index + tekken.json + "
            "preprocessor_config.json). Required — checkpoints are "
    ...
    parser.add_argument(
        "--model-id",
        default=MODEL_ID,
        help="Exact upstream model id recorded in manifest.txt.",
    )
    parser.add_argument(
        "--size",
        default=SIZE,
        help="Stable fixture size label recorded in manifest.txt.",
    )
    parser.add_argument(
        "--request-model",
        default="voxtral-mini-2507",
        help=(
            "Model string supplied to mistral_common TranscriptionRequest. "
            "Use voxtral-small-2507 for the Small-24B release."
        ),
    )
    parser.add_argument(
        "--full-taps",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Stop after upstream mel + audio tower + projector fixtures. "
            "Useful for the first VAST gate before the much heavier text decoder."
        ),
    )
    parser.add_argument(
        "out_dir",
    ...
    pre_audio, n_audio_tokens, post_audio, tok = build_prompt(
        ckpt, audio_path, args.request_model
    )
    print(
    ...
    if args.tower_only:
        manifest["scope"] = "tower_only"
        write_manifest(out_dir, manifest)
        print(f"wrote tower-only fixtures to {out_dir} (RSS peak {rss_gib():.2f} GiB)")
        return

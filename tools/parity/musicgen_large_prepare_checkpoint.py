#!/usr/bin/env python3
"""Prepare a ``facebook/musicgen-large`` checkpoint for the Vokra
``vokra-convert --model musicgen-large`` converter.

Offline side-car (FR-LD-05: no Python / PyTorch ever enters the runtime).

# Why this exists

The upstream ``facebook/musicgen-large`` release ships as a bundle:
LM decoder (~13-14 GB, potentially torch pickle ``pytorch_model.bin`` OR
sharded ``model.safetensors``) + frozen T5-base text encoder (~1 GB) +
paired EnCodec RVQ audio codec (~150 MB) + config side-cars. Total
~19.5 GB per HF cardData primary source (2026-08-01) — the top rung of
the MusicGen family (2.2x sibling MusicGen-Medium 11.4 GB).

Vokra's Rust converter (``crates/vokra-convert/src/models/musicgen_large.rs``)
...
1. **safetensors-native (single or sharded)**: walk
   ...
   ``granite_speech_prepare_checkpoint.py`` +
   ``musicgen_medium_prepare_checkpoint.py`` precedent.

2. **torch pickle (pytorch_model.bin)**: delegate to the shared
   ``bin_to_safetensors.py`` bridge — the SBV2 v2 / SpeechT5-HiFi-GAN /
   DeBERTa v3 large / VoxCPM-0.5B / Fun-CosyVoice3 /
   MusicGen-Medium precedent.
...
# Scale — vast.ai handoff (larger box than sibling medium)

MusicGen-Large is ~19.5 GB. The M1 iMac 16 GB machine cannot safely
convert this class of publish (memory ``[[feedback-large-models-on-vast-ai]]``:
Voxtral-Small-24B 48 GB confirmed swap-death at ~40 GB peak; MusicGen-
Large peak footprint = ~2x file size = ~39 GB with the merge buffer).
Run this script on vast.ai per ``docs/handoff/vast-ai-large-model-publish.md``
— allocate a **48 GB+ box** (larger than the 32 GB box that runs the
sibling MusicGen-Medium ~11.4 GB).
...
2026-07-28 = T4 tier Research-only + sibling MusicGen-Medium landed
2026-08-01). ``publish-one.sh --allow-noncommercial`` gate must fire
before any upload.
...
::
    ...
    uv run tools/parity/musicgen_large_prepare_checkpoint.py \\
        --hf-repo facebook/musicgen-large \\
        --output ./musicgen-large.safetensors

Or point at an already-downloaded checkpoint directory::

    uv run tools/parity/musicgen_large_prepare_checkpoint.py \\
        --checkpoint-dir /path/to/musicgen-large \\
        --output ./musicgen-large.safetensors
"""
...
DEFAULT_HF_REPO = "facebook/musicgen-large"


def _log(msg: str) -> None:
    print(f"[musicgen-large-prep] {msg}", file=sys.stderr, flush=True)
...
def _dtype_ok_for_vokra(dtype_str: str) -> bool:
    ...
    ``yue_bundle_prepare_checkpoint.py`` +
    ``musicgen_medium_prepare_checkpoint.py`` INT-strip posture.
    """
...
def _load_from_safetensors_shards(ckpt_dir: Path) -> dict[str, Any]:
    ...
    if index_path.is_file():
        ...
        # De-dup the shard set — usually 4-8 shards for models of this
        # scale (larger than the 2-4 shards typical of MusicGen-Medium).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare a facebook/musicgen-large checkpoint for the Vokra "
            "vokra-convert --model musicgen-large converter (safetensors-only)."
        )
    )
    ...
    parser.add_argument(
        "--output",
        required=True,
        help="Output safetensors path (e.g. ./musicgen-large.safetensors)",
    )

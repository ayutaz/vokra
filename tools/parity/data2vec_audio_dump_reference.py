#!/usr/bin/env python3
"""Dump an independent official Data2Vec Audio CTC reference fixture."""
...
DEFAULT_MODEL = "facebook/data2vec-audio-base-960h"
DEFAULT_REVISION = "32331f3123e703528918aa688a9a38232d58c872"
...
def main() -> int:
    ...
    model = Data2VecAudioForCTC.from_pretrained(args.model_id, revision=args.revision)
    model.eval()
    input_values = processor(pcm, sampling_rate=sample_rate, return_tensors="pt").input_values
    encoder = model.data2vec_audio(input_values).last_hidden_state
    ...
    write_u32(output / "tokens.u32.bin", folded)
    (output / "text.txt").write_text(text + "\n", encoding="utf-8")
    manifest = {
        "format": "vokra-data2vec-audio-reference-v1",

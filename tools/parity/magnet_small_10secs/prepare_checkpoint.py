...
/ `.th` file) containing the 500M-parameter non-autoregressive masked-LM
transformer, plus a bundled EnCodec 32 kHz audio codec, plus a frozen
T5-base text encoder — total ~2 GB per HF cardData primary source
(2026-08-13). The Rust converter
...
The ~2 GB checkpoint fits comfortably in memory on M1 iMac 16 GB per
memory ``[[feedback-large-models-on-vast-ai]]`` (well below the 8 GB
threshold that pushes work to vast.ai). Peak resident memory is roughly
the whole model (~2 GB) because ``safetensors.torch.save_file`` needs
the full dict for its serialise call. No vast.ai handoff needed for
this release (unlike sibling ``firered_asr_llm_l`` ~16.6 GB or
``musicgen_large`` ~19.5 GB).
...
::
    ...
    ./target/release/vokra-cli convert \\
        ...
        --input /path/to/magnet-small-10secs/flat.safetensors \\
        --output /path/to/out/magnet-small-10secs.gguf

    # Then publish (T4 tier: --allow-noncommercial MANDATORY):
    bash scripts/publish/publish-one.sh \\
        --gguf /path/to/out/magnet-small-10secs.gguf \\
...
gate, MusicGen family / X-Codec-2 / jasco_400m_chords_drums precedent).
"""

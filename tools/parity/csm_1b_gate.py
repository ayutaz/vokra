...
"""Stdlib-only terminal gate for the unresolved CSM-1B composite.

The current approval is intentionally an inspection-only, blocked disposition.
It records the authenticated Apache model/source identities while keeping the
Meta tokenizer/community, Mimi role mapping, dependency policy, API smoke,
native runtime, and parity decisions unresolved.  A signed blocked document
cannot authorize acquisition or execution.
"""
...
MODEL_REPOSITORY = "sesame/csm-1b"
MODEL_REVISION = "c92a71e1c419772e25be7dc14d952c2521a740ab"
...
TRANSFORMERS_REVISION = "945727948c1143a10ac6f7d811aa58bb0d126b5b"
PUBLIC_REPOSITORY = "vokra/csm-1b"
PUBLIC_REVISION = "81613fc840fa995f4c8f1c48749fd731ed6424b8"
BLOCKED_MARKER = "BLOCKED_UNRESOLVED_CSM_1B_COMPOSITE"
...
def _fixture(head: str) -> dict[str, Any]:
    data: dict[str, Any] = {
        "schema": "vokra-csm-1b-approval-v1",
        "status": "BLOCKED",
        ...
        "transformers_tag": TRANSFORMERS_TAG,
        "transformers_revision": TRANSFORMERS_REVISION,
        "public_repository": PUBLIC_REPOSITORY,
        "public_revision": PUBLIC_REVISION,
        "model_license_status": "OWNER_SIGNED_PUBLISHED_APACHE_2",
        "source_license_status": "AUTHENTICATED_APACHE_2",
        "tokenizer_license_status": "BLOCKED_META_COMMUNITY_LICENSE_REVIEW",
        "mimi_role_status": "BLOCKED_MIMI_COMPANION_ROLE_MAPPING",
        "full_audio_generation_status": "BLOCKED_FULL_AUDIO_GENERATION_ROLE_COMPLETENESS",
        "dependency_status": "BLOCKED_UNREVIEWED_TRANSITIVE_POLICY",
        "certifi_policy_status": "BLOCKED_POLICY_REVIEW",
        "tqdm_policy_status": "BLOCKED_POLICY_REVIEW",
        "typing_extensions_policy_status": "BLOCKED_POLICY_REVIEW",
        "numpy_policy_status": "BLOCKED_POLICY_REVIEW",
        "api_smoke_status": "BLOCKED_UNVERIFIED_TRANSFORMERS_API_SMOKE",
...
def validate_approval(raw: bytes, head: str, supplied_sha: str) -> dict[str, Any]:
    ...
    if not isinstance(data, dict) or set(data) != set(expected) or any(not _same(data[key], value) for key, value in expected.items()):
...
def require_blocked_gate(head: str, approval: str, supplied_sha: str, root: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"approval evidence rejected: {error}") from error
    validate_approval(raw, head, supplied_sha)
    try:
    ...
    raise RuntimeError(f"{BLOCKED_MARKER}: tokenizer/community, Mimi role mapping, dependencies, native runtime, and parity remain unresolved")
...
def self_test(source: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="csm-gate-repo-") as repo_tmp, tempfile.TemporaryDirectory(prefix="csm-gate-approval-") as approval_tmp:
        repo = Path(repo_tmp).resolve()
        for args in (("init", "-q"), ("config", "user.name", "CSM self-test"), ("config", "user.email", "csm-self-test@example.invalid")):
        ...
        raw = _raw(_fixture(head)); approval.write_bytes(raw); sha = hashlib.sha256(raw).hexdigest()
        ...
        try:
            ...
            raise AssertionError("blocked approval passed")
        ...
        try: require_blocked_gate(head, str(ignored), sha, repo)
            ...
            if "outside the checkout" not in str(error): raise AssertionError("checkout-local approval wrong rejection")
        else: raise AssertionError("ignored checkout-local approval accepted")
        for bad in ("0" * 64,):
            try: validate_approval(raw, head, bad)
            except RuntimeError: pass
            else: raise AssertionError("wrong SHA accepted")
        wrong_head = _fixture("0" * 40); wrong_raw = _raw(wrong_head)
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("csm_1b_gate self-test: OK")
        return 0

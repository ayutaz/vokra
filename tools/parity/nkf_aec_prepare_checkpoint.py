#!/usr/bin/env python3
"""Flatten a fjiang9/NKF-AEC ``pretrained/nkf.pt`` → safetensors
(coverage-audit-2026-08-03 Wave A).

Offline sidecar tool (FR-LD-05: no Python / PyTorch ever enters the
runtime). The upstream NKF-AEC release ships
``github.com/fjiang9/NKF-AEC/blob/main/pretrained/nkf.pt`` — a
torch-pickle checkpoint whose top level is either the flat state dict
itself or a dict wrapping one under a common key (``state_dict`` /
``model_state_dict`` / ``model``). The Rust converter
(``crates/vokra-convert/src/models/nkf_aec.rs``) consumes safetensors
only, so this script bridges the two:
...
* Loads the ``.pt`` with ``torch.load(..., weights_only=True)`` (the
  ...
  posture the sibling ``dfn3_prepare_checkpoint.py`` / ``csm_dump.py``
  scripts adopt).
* Unwraps a single-level ``state_dict`` container if present (the
  ...
  wrap it; the DFN3 pattern of "unwrap known container keys but never
  execute arbitrary unpickled callables" applies).
...
::
    ...
    cd tools/parity
    uv run python nkf_aec_prepare_checkpoint.py \\
...
def main() -> int:
    ...
    ap.add_argument(
        "--input",
        required=True,
        help="path to fjiang9/NKF-AEC nkf.pt (torch pickle)",
    )
    ap.add_argument(
        "--output",
        required=True,

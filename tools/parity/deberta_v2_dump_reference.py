#!/usr/bin/env python3
"""DeBERTa v2 (JA) standalone clean-room reference dumper (Task 31, SBV2 v2
plan) — HF ``transformers`` ``AutoModel``/``AutoTokenizer`` only.

Sibling of ``sbv2_dump_reference.py`` (Task 30), scoped to *just* the JA
BERT encoder ``SbV2Model`` embeds (``DebertaV2Encoder`` on the Rust side,
``crates/vokra-bert/src/deberta_v2.rs``). Useful for isolating BERT-only
...
DeBERTa v2 is loaded directly from the HF Hub via ``AutoModel``, a real,
...
- **DeBERTa v2** paper (He, Liu, Gao, Chen 2021, arXiv:2006.03654).
- **microsoft/DeBERTa** (MIT) — architecture reference.
- **HuggingFace ``transformers``** (Apache-2.0) ``deberta_v2`` module — used
  directly via ``AutoModel``/``AutoTokenizer`` below, the actual reference
  implementation this dumper runs (no re-implementation, no vendoring: see
  ``tools/parity/utmos_dump_reference.py``'s own module doc and memory
  ``feedback-honest-parity-atol`` for why a self-consistent mirror of the
  architecture would validate nothing — this script sidesteps that trap
  entirely by calling the real upstream module).
- The default ``--hf-repo`` below (structural facts only: config, license —
  never a third party's *code*).

# Default checkpoint

``ku-nlp/deberta-v2-large-japanese-char-wwm`` — the JA BERT encoder design
doc §9's SKU table pins (``docs/superpowers/specs/2026-07-26-sbv2-v2-design.md``
§9, §16). License ``cc-by-sa-4.0`` (Copyleft/ShareAlike variant — verified
live via the HF Hub API by Task 30's own review, recorded in that task's
report). Architecture facts below were verified live against this repo's
raw ``config.json`` on the HF Hub
(``huggingface.co/ku-nlp/deberta-v2-large-japanese-char-wwm/raw/main/config.json``),
...
These four numeric facts (1024/24/16/22012) are used **only** as the
...
::

    {
      ...
      "generator": "tools/parity/deberta_v2_dump_reference.py",
      "hf_repo": "ku-nlp/deberta-v2-large-japanese-char-wwm",
      "revision": null,
      "text": "こんにちは。",
      "seed": 42,
      "n_layers": 24,
...
::
    ...
    python3 tools/parity/deberta_v2_dump_reference.py --output-dir /tmp/dbv2-dump

    # real dump (needs torch + transformers + network access to the HF Hub):
    python3 tools/parity/deberta_v2_dump_reference.py \\
        --text "こんにちは。" --output-dir /tmp/dbv2-dump --do-dump
...
LOG_PREFIX = "[deberta-v2-dump]"
GENERATOR_ID = "tools/parity/deberta_v2_dump_reference.py"
GENERATOR_VERSION = "1.0"

# design doc §9 SKU table / §16 References. Verified to exist, with license
# cc-by-sa-4.0, via the public HF Hub API (2026-07-26, Task 30's own review
# — see sbv2_dump_reference.py's identical constant + citation).
DEFAULT_HF_REPO = "ku-nlp/deberta-v2-large-japanese-char-wwm"
...
def run_dump(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        sys.exit(
            ...
            "authorized DeBERTa v2 reference (design doc §6)."
        )
...
def parse_args(argv: "list[str] | None" = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "DeBERTa v2 (JA) clean-room reference dumper: runs the real HF "
    ...
    parser.add_argument(
        ...
        help=f"HF Hub repo id for the DeBERTa v2 encoder (default: {DEFAULT_HF_REPO}).",
    )
    parser.add_argument(
        "--revision",
        default=None,

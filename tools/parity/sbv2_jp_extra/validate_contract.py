...
def _validate_checkout(value: object) -> None:
    if not isinstance(value, dict):
        raise ValueError("environment.vokra_checkout schema is not exact")
    keys = set(value)
    if keys == {"expected_head", "head", "clean"}:
        expected_head = value["expected_head"]
        head = value["head"]
        if not isinstance(expected_head, str) or not HEX40.fullmatch(expected_head):
            raise ValueError("recorded expected checkout HEAD is malformed")
        ...
        if expected_head != head:
            raise ValueError("recorded checkout HEAD disagrees with expected HEAD")
        if value["clean"] is not True:
            raise ValueError("recorded checkout must be clean")
        return
    if keys != {"root", "head"}:
        raise ValueError("environment.vokra_checkout schema is not exact")
    checkout = _object(value, "environment.vokra_checkout", {"root", "head"})
...
def validate(payload: object) -> None:
    ...
    _validate_checkout(environment["vokra_checkout"])
    activity = _object(
        root["activity"],
        "activity",
...
def self_test() -> None:
    ...
    for label, checkout in (
        (
            "production checkout HEAD mismatch",
            {"expected_head": "a" * 40, "head": "b" * 40, "clean": True},
        ),
        (
            "production checkout dirty",
            {"expected_head": "a" * 40, "head": "a" * 40, "clean": False},
        ),
        (
            "production checkout missing key",
            {"expected_head": "a" * 40, "head": "a" * 40},
        ),
        (
            "production checkout extra key",
            {"expected_head": "a" * 40, "head": "a" * 40, "clean": True, "extra": None},
        ),
    ):
        invalid_checkout = dict(production)
        invalid_checkout["environment"] = dict(production["environment"], vokra_checkout=checkout)
        try:

...
Transformers, a model implementation, or Vokra code, and it has no model or
source download path.  The preceding dependency-only ``uv sync`` may use the
locked package indexes; this auditor itself performs no network requests.
...
LICENSE_TOKENS = ("license", "copying", "notice", "copyright")
...
def contract(project: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    return project_data, lock_data, manifest, {"rows": rows, "contract": contract_data, "gate_sha256": sha256_file(project / "preflight_gate.py")}
...
def inspect_package(row: dict[str, Any], candidates: list[dict[str, Any]]) -> tuple[dict[str, Any], list[str]]:
    key = identity(row["name"], row["version"])
    record = candidates[0] if len(candidates) == 1 else None
    package: dict[str, Any] = {"lock": {"name": row["name"], "version": row["version"], "source": row["source"]}, "installed": None}
    failures: list[str] = []
    if record is None:
    ...
    package["installed"] = {"name": dist.metadata.get("Name"), "version": dist.version, "identity": key, "location": record["location"], **facts, "publisher_license_files": licenses, "native_files": native, "bundled_libraries": native}
    if not licenses:
...
def audit_environment(project: Path) -> dict[str, Any]:
    project_data, _lock, manifest, binding = contract(project)
    rows = binding["rows"]
    ...
    for row in rows:
        if row["source"] == {"virtual": "."}:
            continue
        package, package_failures = inspect_package(row, by_identity.get(identity(row["name"], row["version"]), []))
    ...
    missing_license = sorted(package["lock"]["name"] for package in packages if package["installed"] and not package["installed"]["publisher_license_files"])
    report = {
        "schema": SCHEMA,
        ...
        "environment": {"python": platform.python_version(), "platform": sys.platform, "machine": platform.machine(), "readelf_required": True, "auditor_network_requests": 0},
        ...
        "closure": closure,
        "packages": packages,
        "license_facts": {"packages": len(packages), "publisher_license_files_missing": missing_license, "classification": "raw publisher metadata/files only"},
...
def self_test() -> int:
    ...
    assert is_license_path("pkg/LICENSE.txt") and is_license_path("pkg/COPYING")
    ...
    with __import__("tempfile").TemporaryDirectory(prefix="speecht5-dependency-audit-selftest-") as directory:
        ...
        compact = compact_report({"repository": {"root": str(root), "head": "a" * 40, "clean": True, "audit_script_sha256": "b" * 64}, "contract": {"pyproject_sha256": "c" * 64, "uv_lock_sha256": "d" * 64}, "closure": {"exact": True}, "license_facts": {}, "native_facts": {}, "build_only_facts": {}}, "e" * 64)

...
def require_regular_input(path: Path, label: str) -> None:
    """Reject symlinked or non-regular inputs before reading any bytes."""
    if path.is_symlink() or not path.is_file() or has_symlink_ancestry(path):
        raise ValueError(f"{label} must be a regular file without symlink ancestry: {path}")


def require_output_directory(path: Path) -> None:
    """Re-check the destination directory immediately before each publish."""
    if path.is_symlink() or not path.is_dir() or has_symlink_ancestry(path):
        raise ValueError(f"output directory must be a regular directory without symlink ancestry: {path}")


def write_bytes_no_replace(path: Path, payload: bytes) -> tuple[Path, int, int]:
    """Publish one complete regular file without replacing a raced path.

    The payload is written and fsynced to a unique same-directory temporary
    inode first.  ``link`` then publishes that inode with no replacement
    semantics, so readers never observe a partially written final file.
    """
    require_output_directory(path.parent)
    if path.exists() or path.is_symlink():
        raise FileExistsError(f"output already exists or is symlinked: {path}")
    ...
    while True:
        ...
        nonce = f"{os.getpid()}-{counter}-{os.urandom(8).hex()}"
        temp_path = path.parent / f".{path.name}.tmp-{nonce}"
        try:
    ...
    try:
        with os.fdopen(fd, "wb") as handle:
            temp_info = os.fstat(handle.fileno())
            if not stat.S_ISREG(temp_info.st_mode):
                raise OSError(f"temporary output is not a regular file: {temp_path}")
        ...
        published = True
        owner = (path, temp_info.st_dev, temp_info.st_ino)
    except Exception:
        raise
    finally:
        try:
            ...
            # The final hardlink is already complete.  Do not report a false
            # failure (or roll back the published output) solely because its
            # private temporary directory entry could not be unlinked.
            if not published:
                raise
    return owner


def rollback_created(outputs: list[tuple[Path, int, int]]) -> None:
    """Remove only files created by this invocation, leaving existing paths."""
    for path, device, inode in reversed(outputs):
        try:
            info = path.lstat()
        except FileNotFoundError:
            continue
            ...
            continue
        if stat.S_ISREG(info.st_mode) and info.st_dev == device and info.st_ino == inode:
            try:
                path.unlink()
            except OSError:
                pass
...
def _attempt_write(path: Path, payload: bytes) -> int:
    """Return one for a successful race winner and zero for a loser."""
    try:
...
def parse_path_arg(value: str) -> Path:
    """Reject lexical dot components before Path normalisation hides them."""
    components = value.replace("\\", "/").split("/")
    if any(component in {".", ".."} for component in components):
        raise argparse.ArgumentTypeError("path must not contain '.' or '..' components")
    return Path(value)
...
def verify_file(path: Path, filename: str, size: int, sha256: str) -> None:
    ...
    require_regular_input(path, "checkpoint")
...
def verify_source_root(path: Path) -> dict[str, str]:
    ...
    for relative in required:
        ...
        if source.is_symlink() or not source.is_file() or has_symlink_ancestry(source) or not source.resolve().is_relative_to(path.resolve()):
...
def self_test() -> int:
    ...
    for unsafe in ("./relative", "a/../escape", "a/./nested"):
        try:
            parse_path_arg(unsafe)
        except argparse.ArgumentTypeError:
            pass
        else:
            raise AssertionError(f"lexical dot path unexpectedly accepted: {unsafe}")
    temp_parent = "/private/tmp" if Path("/private/tmp").is_dir() else None
    with tempfile.TemporaryDirectory(prefix="yue-reference-output-", dir=temp_parent) as directory:
        root = Path(directory)
        target = root / "payload.bin"
        write_bytes_no_replace(target, b"payload")
        try:
            write_bytes_no_replace(target, b"tamper")
        except FileExistsError:
            pass
        else:
            raise AssertionError("no-replace output unexpectedly allowed overwrite")
        link = root / "link"
        link.symlink_to(root)
        try:
            write_bytes_no_replace(link / "nested.bin", b"unsafe")
        except (FileExistsError, OSError, ValueError):
            pass
        else:
            raise AssertionError("symlink ancestry unexpectedly accepted")
        assert not list(root.glob(".*.tmp-*")), "successful publish leaked a temporary"

        # Parallel callers may race on the same final path, but exactly one
        # complete payload must win and no private temporary may remain.
        from concurrent.futures import ThreadPoolExecutor

        parallel = root / "parallel.bin"
        with ThreadPoolExecutor(max_workers=8) as executor:
            outcomes = list(
                executor.map(
                    lambda index: _attempt_write(parallel, f"payload-{index}".encode()),
                    range(8),
                )
            )
        assert sum(outcomes) == 1, f"parallel no-clobber winners: {outcomes}"
        assert parallel.read_bytes().startswith(b"payload-")
        assert not list(root.glob(".*.tmp-*")), "parallel publish leaked a temporary"

        # A later output failure rolls back only the output published by this
        # invocation; the pre-existing blocking directory remains untouched.
        first = root / "partial-first.bin"
        blocked = root / "partial-blocked.bin"
        blocked.mkdir()
        created: list[tuple[Path, int, int]] = []
        created.append(write_bytes_no_replace(first, b"first"))
        try:
            write_bytes_no_replace(blocked, b"must-fail")
        except FileExistsError:
            rollback_created(created)
        else:
            raise AssertionError("partial publish did not fail on existing path")
        assert not first.exists(), "partial publish rollback left an owned output"
        assert blocked.is_dir(), "rollback touched a pre-existing path"
        assert not list(root.glob(".*.tmp-*")), "rollback left a temporary"

        # Ownership comes from the temporary inode, not a post-publish lstat.
        # A replacement by another writer must therefore survive rollback.
        replaced = root / "replaced.bin"
        owner = write_bytes_no_replace(replaced, b"owned")
        replacement_source = root / "replacement-source.bin"
        replacement_source.write_bytes(b"third-party")
        replaced.unlink()
        replacement_source.rename(replaced)
        replacement_info = replaced.lstat()
        assert (replacement_info.st_dev, replacement_info.st_ino) != owner[1:]
        rollback_created([owner])
        assert replaced.read_bytes() == b"third-party", "rollback removed a replacement"
        assert not list(root.glob(".*.tmp-*")), "ownership test leaked a temporary"
...
def main() -> int:
    ...
    parser.add_argument("--source-root", type=parse_path_arg)
    parser.add_argument("--codec-checkpoint", type=parse_path_arg)
    parser.add_argument("--decoder-checkpoint", type=parse_path_arg)
    parser.add_argument("--output-dir", type=parse_path_arg)
    ...
    if has_symlink_ancestry(args.output_dir) or args.output_dir.is_symlink() or args.output_dir.exists() and (
    ...
    require_output_directory(args.output_dir)
    ...
    try:
        ...
        output_payloads = (
            (
                "codes.u32le",
                frame_major_codes.numpy().astype("<u4", copy=False).tobytes(order="C"),
            ),
            (
                "features.f32le",
                features[0].numpy().astype("<f4", copy=False).tobytes(order="C"),
            ),
            (
                "backbone.f32le",
                backbone_output[0].contiguous().numpy().astype("<f4", copy=False).tobytes(order="C"),
            ),
            (
                "waveform.f32le",
                waveform.numpy().astype("<f4", copy=False).tobytes(order="C"),
            ),
        )
        for name, payload in output_payloads:
            output_path = args.output_dir / name
            created_outputs.append(write_bytes_no_replace(output_path, payload))
        metadata = {
        ...
        created_outputs.append(write_bytes_no_replace(
            manifest_path,

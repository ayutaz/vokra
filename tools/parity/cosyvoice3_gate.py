"""Stdlib-only pre-input gate for the blocked Fun-CosyVoice3 route."""
...
BLOCKED_MARKER = "BLOCKED_UNRESOLVED_COSYVOICE3_COMPOSITE"
...
def _approval_fixture(expected_head: str) -> dict[str, Any]:
    data: dict[str, Any] = {
        "schema": "vokra-cosyvoice3-approval-v1",
        "status": "BLOCKED",
        ...
        "dependency_status": "BLOCKED_MISSING_LOCK_FORBIDDEN_SOXR",
...
def validate_approval(raw: bytes, expected_head: str, supplied_sha: str) -> dict[str, Any]:
    ...
    keys = {
        ...
        "source_repository", "source_revision", "matcha_repository", "matcha_revision", "license_status",
        "dependency_status", "dataset_status", "native_status", "no_upload", "scope_sha256",
    }
...
def require_blocked_gate(expected_head: str, approval: str, approval_sha256: str, root: Path) -> None:
    ...
    raise RuntimeError(f"{BLOCKED_MARKER}: dataset/dependency/native composite is unresolved; no input or model read is permitted")
...
def self_test(source: Path, input_args: list[str], input_marker: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cosyvoice3-gate-") as repo_tmp, tempfile.TemporaryDirectory(prefix="cosyvoice3-approval-") as approval_tmp:
        repo = Path(repo_tmp).resolve()
        subprocess.run(["git", "-C", str(repo), "init", "-q"], check=True, capture_output=True)
        subprocess.run(["git", "-C", str(repo), "config", "user.name", "CosyVoice3 self-test"], check=True)
        subprocess.run(["git", "-C", str(repo), "config", "user.email", "cosyvoice3-self-test@example.invalid"], check=True)
        ...
        raw = _approval_bytes(_approval_fixture(head)); approval.write_bytes(raw)

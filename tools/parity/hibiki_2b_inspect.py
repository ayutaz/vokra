...
HIBIKI_LICENSE_ROLES={"LICENSE-APACHE":("apache license, version 2.0","without warranties or conditions"),"LICENSE-MIT":("permission is hereby granted, free of charge","the software is provided \"as is\""),"hibiki-rs/LICENSE":("apache license, version 2.0","without warranties or conditions")}
MOSHI_LICENSE_ROLES={"LICENSE-APACHE":("apache license, version 2.0","without warranties or conditions"),"LICENSE-MIT":("permission is hereby granted, free of charge","the software is provided \"as is\""),"client/LICENSE":("permission is hereby granted, free of charge","the software is provided \"as is\""),"moshi/LICENSE":("permission is hereby granted, free of charge","the software is provided \"as is\""),"moshi/LICENSE.audiocraft":("permission is hereby granted, free of charge","the software is provided \"as is\""),"moshi_mlx/LICENSE":("permission is hereby granted, free of charge","the software is provided \"as is\""),"rust/LICENSE":("apache license, version 2.0","without warranties or conditions")}
...
def license_has_markers(path:Path,markers:tuple[str,...])->bool:
...
def source_inventory(root:Path,repo:str,revision:str,roles:tuple[str,...],blockers:list[str])->dict[str,Any]:
 ...
 try:
  ...
  license_roles=HIBIKI_LICENSE_ROLES if repo==HIBIKI_REPOSITORY else MOSHI_LICENSE_ROLES
  ...
  for license_name, markers in license_roles.items():
   ...
   if not license_has_markers(license_path,markers): blockers.append(f"license grant/warranty clauses missing: {license_name}")
...
def self_test()->None:
 ...
 assert CONFIG_FACTS[("model_id","sig")] == "ccef4858" and CONFIG_FACTS[("model_id","epoch")] == 200
 assert len(CONFIG_FACTS) == 44
 assert ("model_id",) not in CONFIG_FACTS and ("epoch",) not in CONFIG_FACTS and ("schedule",) not in CONFIG_FACTS
 assert CONFIG_FACTS[("depformer_weights_per_step_schedule",)] == [0,1,2,3,4,5,6,7,8,8,8,8,8,8,8,8]
 assert MOSHI_LICENSE_ROLES["moshi/LICENSE.audiocraft"] == ("permission is hereby granted, free of charge", "the software is provided \"as is\"")
 with tempfile.TemporaryDirectory(prefix="hibiki-inspect-") as d:
  root=Path(d); license_fixture=root/"LICENSE.audiocraft"; license_fixture.write_text('MIT License\\nPermission is hereby granted, free of charge, to any person obtaining a copy.\\nTHE SOFTWARE IS PROVIDED "AS IS"\\n'); assert license_has_markers(license_fixture,MOSHI_LICENSE_ROLES["moshi/LICENSE.audiocraft"]); assert not license_has_markers(license_fixture,("creative commons","noncommercial"))
  p=root/"x.safetensors"; h=json.dumps({"x":{"dtype":"BF16","shape":[1],"data_offsets":[0,2]}}).encode(); p.write_bytes(struct.pack("<Q",len(h))+h+b"\0\0"); bad=[]; assert safe_header(p,root,1,"BF16",bad)["status"]=="HEADER_ONLY" and not bad

...
def self_test()->None:
 ...
 with tempfile.TemporaryDirectory(prefix="owsm-inspect-") as d:
  ...
  config.write_text("model: wrong\n"); bad=[]; packet=config_evidence(config,root,bad); assert packet["contract_status"]=="BLOCKED_FACTS" and (any("mismatch" in x for x in bad) or any("YAML parse blocked" in x for x in bad))
  ...
  card="---\nlicense: cc-by-4.0\ndatasets:\n- espnet/yodas_owsmv4\n---\nLanguage identification, recognition, translation, timestamp and long-form.\n"; bad=[]; parsed=model_card_frontmatter(card,bad); contract=validate_model_card(parsed,card,bad); assert ((parsed is None and any("YAML parse blocked" in item for item in bad)) or (parsed["datasets"]==["espnet/yodas_owsmv4"] and contract["status"]=="AUTHENTICATED_MODEL_CARD" and not bad))
  bad=[]; string_card="---\nlicense: cc-by-4.0\ndatasets: espnet/yodas_owsmv4\n---\nLanguage identification, recognition, translation, timestamp and long-form.\n"; parsed=model_card_frontmatter(string_card,bad); contract=validate_model_card(parsed,string_card,bad); assert ((parsed is None and any("YAML parse blocked" in item for item in bad)) or (parsed["datasets"]=="espnet/yodas_owsmv4" and contract["status"]=="BLOCKED_MODEL_CARD" and any("README dataset declaration mismatch" in item for item in bad)))

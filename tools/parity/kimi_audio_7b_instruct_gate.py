"""Stdlib-only gate for the unresolved Kimi-Audio inspection path.

The exact approval record is a blocked inspection disposition.  Even after it
is authenticated, ``--enforce`` returns exit 2 and never authorizes model or
source acquisition.
"""
...
APPROVAL_SCHEMA = "vokra-kimi-audio-7b-instruct-blocked-approval-v1"
SCOPE: dict[str, str] = {
    "dataset_status": "BLOCKED_TRAINING_PROVENANCE_UNAUTHENTICATED",
    "dependency_status": "BLOCKED_COMPONENT_DEPENDENCY_LICENSE_REVIEW",
    "glm4_revision": "eb00ce9142e8d98b0ed7c57cd47e0d6d5dce9a1a",
    "glm4_url": "https://github.com/THUDM/GLM-4-Voice.git",
    "license_status": "BLOCKED_MODEL_AND_COMPONENT_LICENSE_REVIEW",
    "model_repository": "moonshotai/Kimi-Audio-7B-Instruct",
    "model_revision": "9a82a84c37ad9eb1307fb6ed8d7b397862ef9e6b",
    ...
    "source_status": "BLOCKED_SOURCE_AND_SUBMODULE_LICENSE_REVIEW",
...
def _git_identity(root: Path, expected_head: str) -> None:
    ...
    if status:
        raise GateError("checkout must be clean before Kimi-Audio helper execution")
...
def enforce_blocked_approval(path: Path | str, approval_sha256: str, expected_head: str, root: Path) -> None:
    ...
    raise GateBlocked("BLOCKED_APPROVAL/INSPECTION_ONLY/NO_UPLOAD: Kimi-Audio facts remain unresolved")
...
def self_test(root: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="kimi-audio-gate-") as directory:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.self_test:
        ...
        self_test(Path.cwd())
        print("kimi_audio_7b_instruct_gate self-test: OK")
        return 0

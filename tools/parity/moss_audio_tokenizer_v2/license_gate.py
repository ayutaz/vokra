...
def lock_rows(lock: dict) -> list[dict]:
    ...
    seen = set()
    for p in packages:
        if not isinstance(p, dict) or not isinstance(p.get("name"), str) or not p["name"] or not isinstance(p.get("version"), str) or not p["version"]:
        ...
        if not isinstance(dependencies, list) or any(not isinstance(dep, dict) or not set(dep) <= {"name", "marker"} or not isinstance(dep.get("name"), str) or not dep["name"] or ("marker" in dep and not isinstance(dep["marker"], str)) for dep in dependencies): raise ValueError("malformed lock dependency row")
...
if __name__ == "__main__":
    ap=argparse.ArgumentParser(); ap.add_argument("--self-test",action="store_true"); ap.add_argument("--lock",type=Path); ap.add_argument("--project",type=Path); ap.add_argument("--manifest",type=Path); ap.add_argument("--approval",type=Path); a=ap.parse_args()

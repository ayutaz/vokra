//! CLI flag parsing.
//!
//! Hand-rolled (no `clap` / `structopt`) because the excluded-workspace
//! `Cargo.lock` should stay honest — a bench binary does not need 40
//! crates of clap machinery for eight flags. The parser accepts both
//! `--flag value` and `--flag=value`; `--help` prints usage to stdout
//! and returns [`ParseError::Help`] so the caller can exit `0`.

use std::fmt;

/// Parsed CLI arguments.
///
/// Field defaults match `docs/m3-15-server-latency-handover.md` § 4
/// byte-for-byte. **Every default is documented** on the field so
/// there are no hidden defaults (per task spec).
#[derive(Debug, Clone, PartialEq)]
pub struct Args {
    /// Server base URL. Default `http://127.0.0.1:8080`.
    pub server: String,
    /// URL path appended to `--server`. Default `/api/tts`.
    pub endpoint: String,
    /// Reference utterance body. Default `"Hello world"` (matches
    /// the in-process bench `SHORT_UTTERANCE`).
    pub text: String,
    ...
    /// TLS session tickets / socket pool warm-up. Default 10.
    pub warmup: usize,
...
impl Args {
    ...
    ///
    /// Kept as a method so unit tests can assert on it without spinning
    /// up ureq.
...
pub enum OutputFormat {
    ...
    /// pipelines and for the `docs/m3-15-server-latency-handover.md`
...
/// CI + owner scripts can react without reading stderr.
///
/// A plain `#[repr(u8)]` enum cannot carry two distinct variants that
/// share the discriminant `0`, so `Ok` doubles as the "help
/// requested" clean-exit signal — `main.rs` uses the [`Self::Ok`]
/// variant for both cases and prints the usage banner when a
/// [`ParseError::Help`] is returned.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
pub enum ExitCode {
    /// Measurement window ran to completion (verdict may still be
    /// FAIL — see module docs). Also used for `--help`: the CLI
    /// prints usage and returns 0.
    Ok = 0,
...
pub const USAGE: &str = "\
vokra-server-bench — HTTP-boundary TTS latency benchmark

USAGE:
    vokra-server-bench [OPTIONS]
    ...
    S: Into<String>,
{
    ...
    // Discard argv[0] (program name). It is fine if the caller already
    // stripped it — see `test_parse_no_argv0`.
    let _prog = iter.next();
    ...
    while let Some(raw) = iter.next() {
        ...
        match flag.as_str() {
            ...
            "--warmup" => {
                // Warmup 0 is legal (skip warmup entirely). Positive-int
                // check does not apply — validate as raw usize.
            ...
            "--timeout-secs" => {
                // 0 is legal only if we want an infinite timeout; we
                // require positive to avoid an accidental "hang forever"
                // configuration.
...
mod tests {
    ...
    fn parse(argv: &[&str]) -> Result<Args, ParseError> {
        // Emulate std::env::args(): first token is the program name.
        let mut with_prog = vec!["vokra-server-bench".to_string()];
    ...
    fn usage_mentions_all_flags() {
        // Guard-rail: every flag documented above appears in USAGE.
        for flag in [
            "--server",
            "--endpoint",
            "--text",
